...
class LocatorResolver:
    ...
    def resolve(self, target: IntentTarget):
        ...
        if target.label:
            ...
            if self._try_visible(loc): return loc
        if target.title:
            loc = self.page.get_by_title(target.title)
            if self._try_visible(loc): return loc
        if target.text:
...
[규칙]
1. target은 role+name, label, title, text 위주로 정의
2. Google 검색 시 action="press_key", value="Enter" 사용
...
""".strip()
        ...
        if action in target_actions:
            ...
            # (4) 검증 (Assertion) - 실패 시 에러 발생
                ...
                if not loc.first.is_visible():
                    raise AssertionError(f"Element not visible: {target.brief()}")
    ...
    def _heal_step(self, page, resolver, step, action, error_text) -> Tuple[bool, str]:
        ...
        # 검증(Assert) 단계는 Healing 대상에서 제외하는 것이 일반적이나,
        # 요소를 못 찾아서(Assert Visible 등) 에러가 난 경우는 Healing을 시도할 수 있음.
        ...
        with sync_playwright() as p:
            ...
            for idx, step in enumerate(healed_scenario, start=1):
                ...
                try:
                    ...
                    if action in ["click", "fill", "check", "hover", "select_option", "assert_text", "assert_visible"]:
