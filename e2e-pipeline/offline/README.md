...
### 해결
**Mac 전용 All-in-One Docker 이미지** (`feat/allinone-mac-host-ollama` 브랜치) — 내부 Ollama 제외, 호스트 Mac 의 Ollama 데몬을 `host.docker.internal:11434` 로 사용. 컨테이너에는 다음만 사전 내장:
...
- Dify 플러그인 `.difypkg` (langgenius/ollama — Ollama 클라이언트만, 모델은 호스트에서 제공)
- nginx (localhost upstream 버전)
- supervisord (PID 1, **11개** 프로세스 관리 — 원본의 12개 중 `ollama` 제외)

> **Ollama 바이너리·모델은 포함되지 않는다.** 빌드 머신에서 `ollama pull` 단계가 없고, 런타임에는 `docker run -e OLLAMA_BASE_URL=http://host.docker.internal:11434` 로 호스트 Ollama 를 가리킨다. Mac 호스트에 Ollama 가 **선행 설치·기동**되어 있어야 한다.
...
| 항목 | 결과 |
|------|------|
| 이미지 크기 | **5-7GB (압축 전)**, 3-5GB (gzip 압축) — 원본 대비 4-5GB 절감 |
| `docker run` 횟수 | 1회 (단, 호스트 Ollama 는 선행 기동) |
| 첫 기동 시간 | **3-5분** (ollama seed 복사 단계 없어 원본 대비 빠름) |
| 이후 기동 시간 | 30-60초 |
| 외부 포트 | 3개 (18080 Jenkins / 18081 Dify / 50001 JNLP) |
| 호스트 RAM 권장 | 16GB+ (컨테이너 ~6GB + 호스트 Ollama + 모델) |
| LLM 성능 | **Metal GPU 가속** (50-80 tok/s, Apple Silicon) — 원본 CPU 모드 대비 30-60배 |
...
#### [3/4] Docker buildx build — 2-stage 멀티 빌드

**무엇을 하는가**: 위에서 받은 hpi/difypkg 와 3개 Dify 이미지를 한 이미지로 융합한다. Mac 브랜치는 내부 Ollama 가 없어 ollama 사전 pull 단계가 없고 원본 대비 20-60분 단축 (10-30분).

**2-stage 구조** ([Dockerfile.allinone](Dockerfile.allinone)):

| Stage | base 이미지 | 추출 대상 | 최종 이미지에 포함되는 것 |
| ----- | ----------- | --------- | ------------------------- |
| 1a. `dify-api-src` | `langgenius/dify-api:1.13.3` | `/app` (Python 앱 + .venv) | `/opt/dify-api` 통째 |
| 1b. `dify-web-src` | `langgenius/dify-web:1.13.3` | `/app` (Next.js 빌드) | `/opt/dify-web` 통째 |
| 1c. `dify-plugin-src` | `langgenius/dify-plugin-daemon:0.5.3-local` | `/app` (Go 바이너리) | `/opt/dify-plugin-daemon` 통째 |
| 2. (final) | `jenkins/jenkins:lts-jdk21` | — | OS 패키지 설치 + 위 3개 산출물 통합 |

> **Ollama 스테이지 없음**: 본 브랜치 (`feat/allinone-mac-host-ollama`) 는 호스트 Ollama 를 사용하므로 `ollama/ollama:latest` 스테이지와 `ollama pull` 단계가 완전히 제거됐다. 원본 `feat/allinone-offline-image` 대비 빌드 시간 대폭 감소, 이미지 크기 4-5GB 절감.

**Stage 2 (런타임 통합)** — Jenkins LTS JDK21 위에 모든 것을 얹는다:
...
├─ COPY: dify-api / dify-web / dify-plugin-daemon (Stage 1)
...
> Ollama 바이너리·모델 seed·supervisord `[program:ollama]` 전부 미포함. 호스트 Ollama 가 런타임 전제.
...
| 0:00-0:05 | (출력 짧음 — Ollama 모델 복사 단계 **없음**) | Jenkins/Dify 플러그인·PG snapshot 만 복사 |
| ~0:05 | `[entrypoint-allinone] seed 완료.` | 복사 끝, supervisord 기동 직전 |
| 0:05 | `[entrypoint-allinone] supervisord 기동...` | **11개** 프로세스 순차 시작 (ollama 제외) |
| 0:05-1:00 | `postgresql ... ready` / `redis ... ready` / `qdrant ... listening` / dify-plugin-daemon ready | 각 서비스 초기화 |
| 1:00-2:00 | `[HH:MM:SS] [▶] === 1. 서비스 헬스체크 ===` | `provision-apps.sh` 시작. Dify/Jenkins HTTP 200 대기 + 호스트 Ollama 도달 확인 |
| 2:00-3:00 | `[▶] === 2. Dify 초기 설정 ===` → `2-3a. Ollama 플러그인 설치` → `2-3b. 모델 공급자 등록` → `2-3c. credential_id swap` → `2-3d. Redis FLUSH` → `2-3e. dify-api 재기동` | Dify REST + credential 스왑 + Redis 캐시 정리 |
| 3:00-4:00 | `[▶] === 2-4. Chatflow DSL import ===` → Publish → API Key 발급 | App import → publish (`-d "{}"`) → 토큰 발급 |
| 4:00-5:00 | `[▶] === 3. Jenkins 초기 설정 ===` → 플러그인 검증 → Credentials → Pipeline Job → Node (`updateNode` 로 디스크 flush) | Jenkins REST/Groovy |
| ~5:00 | `[▶] === 프로비저닝 완료 ===`  ·  `[entrypoint-allinone] 앱 프로비저닝 완료.`  ·  `[entrypoint-allinone] 준비 완료. supervisord wait...` | 사용 가능 상태 |

**재기동** (`docker restart dscore-qa`): 30-60초. `/data/.initialized` + `/data/.app_provisioned` 플래그 덕분에 seed 와 provision 모두 스킵.

> 원본 `feat/allinone-offline-image` 의 10분 기동 타임라인 대비 **절반 이하** (Ollama 4GB 모델 복사 + 내부 Ollama 기동 + Ollama 모델 호출 테스트 단계가 모두 제거됨).
...
| 3 | Ollama 모델 등록 | **호스트(Mac)** 에서 `ollama list` 에 `gemma4:e4b` + `docker exec dscore-qa bash -c "PGPASSWORD=difyai123456 psql -h 127.0.0.1 -U postgres -d dify -tAc \"SELECT model_name FROM provider_models WHERE provider_name LIKE '%ollama%'\""` 에 `gemma4:e4b` | Dify Settings → Model Provider → Ollama 에 `gemma4:e4b` | [2.6-B](#b-ollama-모델-등록-실패) |
...
- **증상**: `[⚠] Ollama 모델 등록 응답 이상` 또는 `credentials POST 이상 응답`
- **원인** (Mac 브랜치): 호스트 Ollama 가 미기동이거나 `OLLAMA_MODEL` env 와 호스트의 실제 모델명이 불일치.
- **복구**:

  ```bash
  # Mac 터미널에서
  ollama list                                  # gemma4:e4b (또는 OLLAMA_MODEL 값) 존재 확인
  brew services restart ollama                 # 데몬 재기동
  curl -fsS http://127.0.0.1:11434/api/tags    # 응답 확인

  # 컨테이너 안에서 provision 재실행 (credential swap + Redis FLUSH 자동 수행)
  docker exec dscore-qa bash /opt/provision-apps.sh
  ```
...
## 4. Ollama 모델 관리 (Mac 전용 — 호스트 Ollama 기반)

> **Mac 브랜치 이미지는 컨테이너 내부에 Ollama 가 없다.** 모든 모델 관리는 **macOS 호스트의 Ollama** 로 수행한다. 즉 `ollama` 명령은 **호스트 터미널에서** 실행. 컨테이너 내부 `docker exec ... ollama` 는 `command not found` 로 실패한다. 아래 명령은 특별한 주석이 없는 한 **Mac 터미널** 에서 실행.

### 4.1 현재 상태 확인

```bash
# (1) 호스트 Ollama 데몬에 탑재된 모델 목록 + 크기 (Mac 터미널)
ollama list
...
# (3) 호스트 Ollama 모델 저장 위치 (macOS)
du -sh ~/.ollama/models 2>/dev/null

# (4) 호스트 Ollama 데몬 상태 / 로그
brew services list | grep ollama   # (Homebrew 설치 시)
tail -30 ~/.ollama/logs/server.log 2>/dev/null

# (5) Dify 의 Ollama 프로바이더에 등록된 모델 목록 (컨테이너 안에서 조회)
docker exec dscore-qa bash -c "
  PGPASSWORD=difyai123456 psql -h 127.0.0.1 -U postgres -d dify -tAc \"
    SELECT pm.model_name, pmc.encrypted_config
      FROM provider_models pm
      JOIN provider_model_credentials pmc ON pm.credential_id = pmc.id
     WHERE pm.provider_name LIKE '%ollama%';\"
"
# 예상: gemma4:e4b|{"base_url": "http://host.docker.internal:11434", ...}
```

> DB 조회가 Dify REST + 쿠키 방식보다 안정적이다. Dify 의 Ollama provider 는 컨테이너 안에 있어도 실제 모델 실행은 호스트에서 일어난다는 점만 기억.

### 4.2 새 모델로 교체 — 호스트 pull + Dify 재등록

Mac 브랜치는 컨테이너 내부에 Ollama 가 없으므로 **재빌드가 필요 없다**. 호스트에 새 모델을 pull 하고 컨테이너 기동 시 `OLLAMA_MODEL` env 만 다른 이름으로 주입하면 된다.

```bash
# (1) Mac 터미널 — 새 모델 pull
ollama pull llama3.1:8b
ollama list   # 호스트에 llama3.1:8b 보여야 함

# (2) 호스트에서 직접 시험 실행 (Metal 가속 확인)
...
# (4) 프로비저닝 강제 재실행 — base_url 및 모델 id 를 Dify 에 다시 반영
docker exec dscore-qa rm -f /data/.app_provisioned
docker restart dscore-qa
# 또는 provision-apps.sh 만 수동 재실행:
docker exec dscore-qa bash /opt/provision-apps.sh
```

**주의 — Chatflow DSL 의 모델 id 하드코딩**: [dify-chatflow.yaml](../dify-chatflow.yaml) 의 LLM 노드에는 `gemma4:e4b` 같은 모델 id 가 **DSL 안에 박혀** 있다. `OLLAMA_MODEL` env 를 바꾸면 Dify 의 provider 등록 모델은 바뀌지만, 실제 Chatflow 실행 시엔 DSL 안의 옛 모델 id 를 계속 참조한다. 두 가지 해결 경로:

- 4.5 UI 경로로 Chatflow 의 LLM 노드 Model 을 드롭다운에서 재선택 → Publish
- 빌드 머신에서 DSL yaml 을 직접 수정하고 번들 재빌드
...
| `llama3.1:70b` | ~40GB | 80GB+ | 고사양 전용 — **Mac 16GB 에서 불가** |

### 4.3 (삭제됨) 런타임에 모델 추가 투입 — 오프라인 전송

Mac 브랜치는 호스트 Ollama 기반이라 "오프라인 전송" 절차가 필요 없다. 호스트 Mac 에서 직접 `ollama pull` 로 받으면 된다.

폐쇄망 Linux 서버에서 오프라인 모델 전송이 필요하다면 원본 `feat/allinone-offline-image` 브랜치의 README §4.3 을 참조.

### 4.4 Dify 에 신규 모델 등록

Ollama 에 모델이 올라가도 Dify 의 드롭다운에는 자동으로 뜨지 않는다. 공급자 레벨에서 한 번 등록해야 한다.

#### (a) 자동 — provision-apps.sh 재실행 (권장)

새 `OLLAMA_MODEL` env 로 컨테이너를 재생성 → provision-apps.sh 가 자동으로 credential 추가 + swap + Redis FLUSH 까지 수행. 4.2 단계 참조.
...
4. 입력:
   ...
   - **Base URL**: `http://host.docker.internal:11434`  **← 반드시 host.docker.internal**
...
> UI 로 수동 추가한 credential 은 2-3c credential swap 을 거치지 않으므로 드물게 `provider_models.credential_id` 가 옛 값으로 남을 수 있다. Chatflow 실행 시 옛 모델로 호출되면 섹션 [5 트러블슈팅](#5-트러블슈팅) 의 "Dify 가 여전히 옛 base_url 로 호출" 항목 참조.

#### (c) CLI 경로 — 자동화 스크립트 내장용

provision-apps.sh 의 2-3b 단계와 동일한 REST 호출 (컨테이너 내부):
...
# 쿠키 재로그인 (만료된 경우)
PW_B64=$(printf "Admin1234!" | base64)
curl -sS -c $COOKIES \
  -X POST http://127.0.0.1:18081/console/api/login \
  -H "Content-Type: application/json" \
  -d "{\"email\":\"admin@example.com\",\"password\":\"$PW_B64\",\"remember_me\":true}" \
  > /dev/null
...
# credential swap 수동 수행 (POST 는 새 레코드 추가만 하므로)
PGPASSWORD=difyai123456 psql -h 127.0.0.1 -U postgres -d dify <<SQL
   ...
   SET credential_id = (
         SELECT id FROM provider_model_credentials
          WHERE provider_name = pm.provider_name
            ...
            AND encrypted_config LIKE '%host.docker.internal%'
          ORDER BY updated_at DESC LIMIT 1)
 WHERE provider_name LIKE '%ollama%' AND model_name = '$MODEL';
SQL

# Redis 캐시 FLUSH
redis-cli -h 127.0.0.1 --scan --pattern 'provider_model_credentials:*' | xargs -r redis-cli -h 127.0.0.1 DEL
supervisorctl restart dify-api dify-plugin-daemon
SH
```

성공 시 HTTP 200/201. 실패 체크리스트:

- 403 → CSRF 토큰 추출 실패. 쿠키 파일 (`cat /tmp/dify-cookies.txt`) 에 `csrf_token` 라인 확인
- 400 `model not found` → 호스트 Ollama 에 그 이름 없음. Mac 터미널에서 `ollama list` 로 재확인
...
> **경고**: Chatflow 가 참조 중인 모델을 삭제하면 Pipeline 실행 시 `model "...": not found` 으로 실패한다. 삭제 전 [4.5](#45-chatflow-에서-사용-모델-변경) 로 먼저 교체.

### 4.7 Ollama 런타임 설정 튜닝

Mac 호스트의 Ollama 데몬 설정은 **호스트 환경변수** 로 제어한다 (컨테이너 안에는 튜닝 대상이 없음).

| 환경변수 | 기본 | 의미 |
| ------- | ---- | ---- |
| `OLLAMA_HOST` | `127.0.0.1:11434` | 바인드 주소. Docker Desktop Mac 은 `host-gateway` 로 127.0.0.1 도달 가능 (변경 불필요). Linux 서버 배포 시 `0.0.0.0:11434` 필요 |
| `OLLAMA_MAX_LOADED_MODELS` | `1` | 동시에 메모리에 올릴 모델 수. RAM 여유 있으면 2-3 |
| `OLLAMA_KEEP_ALIVE` | `5m` | 모델 언로드 지연. `-1` 이면 영구 상주 |
| `OLLAMA_NUM_PARALLEL` | `1` (버전별) | 동시 요청 수 |
| `OLLAMA_FLASH_ATTENTION` | 미설정 | `1` 로 설정 시 FA 활성 (지원 모델 한정) |

**호스트(Mac)에서 설정하는 방법**:

```bash
# (a) Homebrew 서비스 — launchctl plist
brew services stop ollama
launchctl setenv OLLAMA_KEEP_ALIVE -1
launchctl setenv OLLAMA_MAX_LOADED_MODELS 2
brew services start ollama

# (b) foreground 실행 (디버깅용)
OLLAMA_KEEP_ALIVE=-1 OLLAMA_MAX_LOADED_MODELS=2 ollama serve
```

Dify 쪽 `context_size` / `max_tokens` 는 [provision-apps.sh:49-50](provision-apps.sh#L49-L50) 의 `OLLAMA_CONTEXT_SIZE`, `OLLAMA_MAX_TOKENS` 환경변수로 — 첫 기동 `docker run` 에 `-e OLLAMA_CONTEXT_SIZE=16384` 형태로 주입하거나, UI ([4.4(b)](#b-ui-경로--일회성-수동-추가)) 에서 해당 모델 행을 열어 수정.
...
> **중요**: Mac 브랜치 이미지는 **컨테이너 내부 Ollama 가 완전히 제거**된 이미지다 (바이너리·모델 seed·supervisord 프로그램 모두 미포함). 호스트 Ollama 가 돌지 않거나 `OLLAMA_BASE_URL` 이 잘못 설정되면 Jenkins Pipeline Stage 3 가 `Connection refused` 로 실패한다. 이 이미지의 기본 가정은 **"호스트 Ollama 가 항상 켜져 있음"**.
...
| 변수 | 효과 |
| ---- | ---- |
| `OLLAMA_BASE_URL=http://host.docker.internal:11434` | Dify 의 Ollama 프로바이더가 호출할 호스트 Ollama URL. Mac 이미지 기본값은 `host.docker.internal:11434` 이라 생략 가능 (명시 권장). 사내 Ollama 게이트웨이 같은 다른 엔드포인트를 쓰고 싶을 때만 변경. |
| `OLLAMA_MODEL=gemma4:e4b` | Dify 에 등록할 모델 id. 호스트에 실제로 pull 된 이름과 **정확히 일치**해야 한다 (`ollama list` 결과 그대로). 기본값 `gemma4:e4b`. |

> `SKIP_INTERNAL_OLLAMA` env 는 Mac 이미지에서 의미가 없다 — 애초에 내부 Ollama 가 없으므로 무시된다 (호환성 위해 entrypoint 가 경고만 출력).

**주의 — 기존 볼륨으로 전환 시**: `dscore-data` 볼륨에 이전 내부-Ollama 이미지의 `.app_provisioned` / `gemma4:e4b` 세부 데이터가 남아있다면 자동 재프로비저닝이 되지 않는다. 첫 기동이 아닌 경우 단계 5 를 참고해 강제 재실행.

#### 단계 4 — 동작 검증

```bash
# 4-1. 내부 Ollama 가 "없어야" 정상
docker exec dscore-qa supervisorctl status 2>&1 | grep -i ollama || echo "[OK] ollama 프로그램 없음"
docker exec dscore-qa which ollama 2>&1 || echo "[OK] ollama 바이너리 없음"

# 4-2. 컨테이너에서 호스트 Ollama 도달
docker exec dscore-qa curl -fsS http://host.docker.internal:11434/api/tags \
  | python3 -c "import json,sys; d=json.load(sys.stdin); print('호스트 모델:', [m['name'] for m in d.get('models',[])])"

# 4-3. Dify 프로바이더에 등록된 base_url (provision-apps.sh 의 2-3c swap 이후)
docker exec dscore-qa bash -c "
  PGPASSWORD=difyai123456 psql -h 127.0.0.1 -U postgres -d dify -tAc \"
    SELECT pmc.encrypted_config
      FROM provider_models pm
      JOIN provider_model_credentials pmc ON pm.credential_id = pmc.id
     WHERE pm.provider_name LIKE '%ollama%';\"
"
...
print(f\"토큰/초: {d['eval_count']/d['eval_duration']*1e9:.1f}\")
"
# Apple Silicon (M1 Pro 이상): 40-80 토큰/초
```

#### 단계 5 — 프로비저닝 재실행 (기존 볼륨 사용 시)

기존 볼륨에 옛 이미지 (내부 Ollama 버전) 시절의 `127.0.0.1:11434` credentials 이 남아있다면, 새 이미지의 provision-apps.sh 를 다시 돌려 Mac 호스트 URL 로 강제 재등록:

```bash
# (a) provision 재실행 — 2-3c swap + 2-3d Redis FLUSH 가 자동 수행됨
docker exec dscore-qa bash /opt/provision-apps.sh
...
그 후 단계 4-3 의 DB 조회로 `host.docker.internal` 로 갱신됐는지 검증. 실패하면 섹션 [5 트러블슈팅](#5-트러블슈팅) 의 "Dify 가 여전히 옛 base_url 로 호출" 항목 참조.

#### 단계 6 — Jenkins Pipeline 실행

Dashboard → `DSCORE-ZeroTouch-QA-Docker` → Build with Parameters → HEADLESS 체크 → Build.
Stage 3 가 **15-30초 내** Planner 응답 완료되면 성공 (내부 CPU Ollama 시절 5분+ timeout 대비 10-20배 단축).

첫 Build 시 **HEADLESS=true 필수** — 컨테이너에 X display 가 없어 headed Chromium 은 즉시 실패한다.

#### 자주 하는 실수

- **호스트에 Ollama 미설치** — `ollama list` 명령 자체가 실패. `brew install ollama && brew services start ollama` 로 준비.
- **`--add-host host.docker.internal:host-gateway` 누락** — Docker Desktop Mac 은 생략해도 자동 해석되지만 Linux 서버 이식을 위해 항상 포함.
...
- **모델 이름 불일치** — `OLLAMA_MODEL=gemma4:e4b` 인데 호스트에 `gemma4:latest` 만 있으면 Dify provider 에서 "model not found". `ollama list` 의 **NAME 컬럼** 그대로 복사.
- **볼륨 전환 시 재프로비저닝 안 함** — OLLAMA_BASE_URL 을 바꿨는데도 Dify 가 여전히 옛 값으로 호출. 단계 5 로 수동 재실행.
- **Windows WSL2 / Linux 서버에서 이 이미지 사용** — Mac 전용 이미지라 GPU 컨테이너 passthrough 경로가 막혀 있다. 해당 환경은 `feat/allinone-wsl-windows` 또는 원본 `feat/allinone-offline-image` 브랜치 이미지를 사용.
...
**Mac 브랜치 이미지에서 Stage 3 실패의 대표 원인 3가지**:

1. **호스트 Ollama 미기동**: Mac 터미널에서 `ollama list` 실패 또는 `curl http://127.0.0.1:11434/api/tags` 실패.
   - 해결: `brew services start ollama` 또는 메뉴바 Ollama 앱 실행.
2. **모델 이름 불일치**: Dify 에 등록된 모델 이름과 호스트 Ollama 의 실제 모델 이름이 다름.
   - 진단: `ollama list` 의 NAME 컬럼 vs `docker exec dscore-qa bash -c 'PGPASSWORD=difyai123456 psql -h 127.0.0.1 -U postgres -d dify -tAc "SELECT model_name FROM provider_models WHERE provider_name LIKE %ollama%"'`
   - 해결: 양쪽을 정렬. 4.4(a) 로 provision 재실행 또는 4.4(b) 로 UI 수동 추가.
3. **`--add-host` 누락**: Docker Desktop Mac 은 자동 해석하지만 Linux 에서는 필수. 진단:
   - `docker exec dscore-qa curl -fsS http://host.docker.internal:11434/api/tags` 실패 시 `docker run` 에 `--add-host host.docker.internal:host-gateway` 재추가.

### Dify 가 여전히 옛 base_url 로 호출 (127.0.0.1:11434 → Connection refused)

- **증상**: `OLLAMA_BASE_URL=http://host.docker.internal:11434` 로 컨테이너를 재생성했는데도 dify-api 로그에 `HTTPConnectionPool(host='127.0.0.1', port=11434): Connection refused` 가 계속 찍힘.
- **원인**: Dify 의 credential 저장 구조 때문. `POST /model-providers/.../models/credentials` 는 **새 credential 레코드만 추가**하고 기존 `provider_models.credential_id` 포인터는 갱신하지 않는다. 또한 Redis 에 `provider_model_credentials:*` 키로 base_url 을 캐시한다. 컨테이너 재기동만으로는 둘 다 해결되지 않음.
- **정상 플로우**: provision-apps.sh 의 2-3b (POST credentials) → 2-3c (`credential_id` swap via SQL UPDATE) → 2-3d (Redis FLUSH) → 2-3e (dify-api/plugin-daemon 재기동) 를 연쇄로 수행해야 반영됨.
- **수동 복구**:

  ```bash
  docker exec dscore-qa bash -c '
    # 1) credential_id 를 host.docker.internal 레코드로 swap
    PGPASSWORD=difyai123456 psql -h 127.0.0.1 -U postgres -d dify <<SQL
    UPDATE provider_models pm
       SET credential_id = (
             SELECT id FROM provider_model_credentials
    ...
    # 2) Redis 캐시 삭제
    redis-cli -h 127.0.0.1 --scan --pattern "provider_model_credentials:*" \
      | xargs -r redis-cli -h 127.0.0.1 DEL

    # 3) dify-api + plugin-daemon 재기동
    supervisorctl restart dify-api dify-plugin-daemon
  '
  ```

  가장 간단한 경로는 **provision-apps.sh 재실행** — 위 3단계를 자동으로 수행한다:
...
└── logs/                     # 모든 서비스 로그
    ...
    └── ...   # ollama.log 는 없음 (호스트 Mac 의 ~/.ollama/logs/ 참조)
```
...
3. `bash /opt/provision-apps.sh` 호출 (오프라인 전용 독립 스크립트 — 기존 setup.sh 와 분리)
   - 환경변수: `DIFY_URL=http://127.0.0.1:18081`, `JENKINS_URL=http://127.0.0.1:18080`,
     `OFFLINE_DIFY_PLUGIN_DIR=/opt/seed/dify-plugins`, `OLLAMA_BASE_URL=http://host.docker.internal:11434` (docker run -e 로 오버라이드 가능)
   - 수행: localhost 헬스체크 → Dify 관리자/로그인 → 로컬 .difypkg 업로드 →
     모델 등록 (POST) → **credential_id swap (SQL)** → **Redis `provider_model_credentials:*` FLUSH** → dify-api/plugin-daemon 재기동 →
     Chatflow import/Publish (`-d "{}"`)/API Key 발급 → Jenkins 플러그인 로드 검증 →
     Credentials 등록 → Pipeline Job 생성 → Node 등록 (**`updateNode()` 로 디스크 flush**)
