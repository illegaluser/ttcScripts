...
> **1-2단계 생략**: brew 만 설치되어 있다면 **5단계 스크립트에 `AUTO_INSTALL_DEPS=true`** 를 넘겨
> ollama + 모델 pull + openjdk@21 + python@3.12 를 한 번에 자동 설치할 수 있습니다. 이미 설치된
> 항목은 스킵됩니다.
...
스크립트 동작 (7단계, idempotent — 설치된 것은 스킵):

1. Ollama 데몬 + 모델 확인 (`AUTO_INSTALL_DEPS=true` 시 `brew install ollama` + `services start` + `ollama pull $OLLAMA_MODEL`)
2. JDK 21 확인 (`AUTO_INSTALL_DEPS=true` 시 `brew install openjdk@21`)
3. Python 3.11+ 확인 (`AUTO_INSTALL_DEPS=true` 시 `brew install python@3.12`)
4. `~/.dscore.ttc.playwright-agent/venv` 생성 + `pip install requests playwright pillow` + `playwright install chromium` → `~/Library/Caches/ms-playwright/`
...
스크립트 수행 (7단계, idempotent, 5-10분 최초):

1. **Ollama 데몬 + 모델 확인** — 없으면 `AUTO_INSTALL_DEPS=true` 로 `brew install ollama` / `brew services start ollama` / `ollama pull $OLLAMA_MODEL` 자동 수행
2. **JDK 21 확인** — 없으면 중단 + 설치 안내 (또는 `AUTO_INSTALL_DEPS=true` 로 `brew install openjdk@21`)
3. **Python 3.11+ 확인** — 없으면 중단 (또는 `AUTO_INSTALL_DEPS=true` 로 `brew install python@3.12`)
4. **venv + Playwright Chromium** — `~/.dscore.ttc.playwright-agent/venv` 생성 + `pip install requests playwright pillow` + Chromium → `~/Library/Caches/ms-playwright/` (macOS arm64 네이티브)
5. **Jenkins Node remoteFS 절대경로 갱신** + workspace venv 심볼릭 링크
...
| `true` | 누락된 ollama/모델/openjdk@21/python@3.12 를 `brew install` / `ollama pull` 로 자동 설치. Homebrew 필수 |
...
호스트 쪽: agent 터미널 `Ctrl+C` / `rm -rf ~/.dscore.ttc.playwright-agent` 로 완전 정리.
...
docker image inspect dscore.ttc.playwright:latest --format '{{.Architecture}}'  # arm64 확인
```
...
이하 4 개가 **사용자가 직접 호출하거나 동작을 이해할 필요가 있는** 핵심 스크립트.
...
| `TARGET_PLATFORM` | `uname -m` 자동감지 | arm64 → `linux/arm64`. amd64 override 시 qemu silent-fail 주의 |
...
### 6.4 `mac-agent-setup.sh` — 호스트 Mac agent 셋업 ⭐

**이 프로젝트의 주된 UX 지점.** 호스트 Mac 의 JDK/Python/venv/Playwright/agent.jar 를 idempotent 하게 준비하고 foreground 로 agent 기동한다.
...
| 단계 | 동작 | 스킵 조건 |
| --- | --- | --- |
| 1 | Ollama 데몬 + 모델 확인 | API 200 + `ollama list` 에 해당 모델 NAME 존재 |
| 2 | JDK 21 탐지 (엄격) | `/opt/homebrew/opt/openjdk@21/bin/java` 또는 `/usr/libexec/java_home -v21` |
| 3 | Python 3.11+ 탐지 | `sys.version_info >= (3,11)` 를 만족하는 `python3` |
| 4 | venv 생성 + `pip install requests playwright pillow` + `playwright install chromium` | venv 존재 / Chromium 캐시 존재 |
| 5 | Jenkins Node `mac-ui-tester` 의 remoteFS 를 **절대경로**로 Groovy 갱신 + workspace venv 심볼릭 링크 | — 매번 실행 (idempotent) |
| 6 | `agent.jar` 다운로드 | 파일 존재 (`FORCE_AGENT_DOWNLOAD=true` 로 강제) |
| 7 | **기존 agent.jar 프로세스 감지 → SIGTERM → Jenkins disconnect 인지 대기 → 새 agent foreground 기동** | — |
...
| `AUTO_INSTALL_DEPS` | `false` | `true` 면 brew install ollama / openjdk@21 / python@3.12 + ollama pull |
| `OLLAMA_MODEL` | `gemma4:e4b` | 존재 확인 대상 모델 |
| `JENKINS_URL` | `http://localhost:18080` | |
| `AGENT_NAME` | `mac-ui-tester` | Jenkins Node 이름 |
| `MAC_AGENT_WORKDIR` | `$HOME/.dscore.ttc.playwright-agent` | 호스트 agent 작업 디렉토리 |
...
~/.dscore.ttc.playwright-agent/            (호스트 Mac)
