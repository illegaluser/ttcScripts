...
if [ ! -f "$DATA/.initialized" ]; then
  ...
  mkdir -p \
    ...
    "$DATA/logs"
  # NOTE: /data/jenkins-agent 는 더 이상 사용 안 함 — agent 는 호스트 (Mac/Windows) 에서 실행

  # Ollama 모델 seed 제거됨 — 호스트 Ollama 를 사용하므로 컨테이너에 모델 파일이
  # 포함되지 않는다. 연결 경로는 OLLAMA_BASE_URL env 로 지정.
...
# (내부 Ollama 는 제거됐으므로 OLLAMA_MODELS 는 불필요.)
...
if [ ! -f "$DATA/.app_provisioned" ]; then
  ...
  if [[ "${OLLAMA_BASE_URL}" != *"host.docker.internal"* ]] && \
    ...
    warn "  이 이미지에는 내부 Ollama 가 없습니다. localhost 로 설정된"
...
# ────────────────────────────────────────────────────────────────────────────
# 5. 호스트 agent 연결 안내 (컨테이너 내부 agent 는 제거됐음 — 하이브리드 설계)
#
# 원본 설계 복원 — Jenkins agent 는 호스트 (Mac / Windows) 에서 직접 실행해야
# Playwright 가 OS 화면에 Chromium 창을 띄울 수 있다 (시각 검증). 컨테이너는
# JNLP 포트 50001 만 노출하고, 호스트 agent.jar 가 외부에서 이 포트로 접속한다.
# ────────────────────────────────────────────────────────────────────────────
if [ -f "$DATA/.app_provisioned" ]; then
