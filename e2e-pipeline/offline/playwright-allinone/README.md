...
> 상세 옵션은 [§2.1 build.sh](#21-buildsh) 참조.
...
```bash
./build.sh --redeploy              # 볼륨 유지 (provision 재사용)
./build.sh --redeploy --fresh      # 볼륨까지 초기화
./build.sh --redeploy --no-agent   # 컨테이너만 재기동
```

폐쇄망 타겟 (빌드 머신 ≠ 실행 머신) 에서는 `--redeploy` 를 쓰지 않고 **Step 1 만** 수행한 뒤 tar.gz 를 옮겨 실행 머신에서 **Step 2 → Step 3** 을 수행한다.

옵션 상세: [§2.1 build.sh](#21-buildsh).

---

## 2. 구성 파일 레퍼런스

`playwright-allinone/` 디렉토리의 모든 파일을 사용자가 실행/접근하는 순서에 맞춰 나열. 각 섹션 포맷은 일관:
...
**실행 시점**: `build.sh` 의 [3/4] 단계가 호출하는 `docker buildx`.
