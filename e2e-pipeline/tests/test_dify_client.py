...
requests.post 를 mock 하여 네트워크 없이 테스트한다.
"""
...
@patch("zero_touch_qa.dify_client.requests.post")
def test_upload_file_success(mock_post, client, tmp_path):
    """정상 업로드 시 file_id 를 반환한다."""
    mock_post.return_value = _make_response(200, {"id": "file-123"})
    f = tmp_path / "doc.pdf"
...
@patch("zero_touch_qa.dify_client.requests.post")
def test_upload_file_http_error(mock_post, client, tmp_path):
...
@patch("zero_touch_qa.dify_client.requests.post")
def test_generate_scenario_success(mock_post, client):
...
@patch("zero_touch_qa.dify_client.requests.post")
def test_generate_scenario_parse_failure(mock_post, client):
    """JSON 파싱 불가 시 DifyConnectionError 가 발생한다."""
    mock_post.return_value = _make_response(200, {"answer": "no json here"})
...
@patch("zero_touch_qa.dify_client.requests.post")
def test_generate_scenario_with_file_id(mock_post, client):
...
@patch("zero_touch_qa.dify_client.requests.post")
def test_request_healing_success(mock_post, client):
    """치유 응답에서 dict 를 반환한다."""
...
@patch("zero_touch_qa.dify_client.requests.post")
def test_request_healing_returns_none(mock_post, client):
    """파싱 불가 시 None 을 반환한다."""
    mock_post.return_value = _make_response(200, {"answer": "unable to heal"})
    result = client.request_healing("에러", "<html></html>", {"action": "click"})
...
@patch("zero_touch_qa.dify_client.requests.post")
def test_call_timeout(mock_post, client):
    """API 타임아웃 시 DifyConnectionError 가 발생한다."""
