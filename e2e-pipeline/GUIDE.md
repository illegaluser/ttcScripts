...
**왜?** setup.sh 는 Jenkins 컨트롤러에 `mac-ui-tester` 노드 메타데이터만 등록한다. 실제로 빌드를 수행할 에이전트 프로세스(`agent.jar`)는 **에이전트 머신 위에서 직접 실행**해야 한다 (Mac, 또는 Playwright 가 동작할 다른 호스트).

**준비물 (에이전트 머신):**
- Java 11 이상 (`java -version` 으로 확인. 없으면 macOS 는 `brew install openjdk@17`)
- Python 3.9 이상 + Playwright (`pip3 install playwright && playwright install chromium`)

**어떻게?**

setup.sh 종료 시 아래와 같은 블록이 출력된다:

```
5. agent.jar 다운로드 및 연결:

     curl -O http://localhost:18080/jnlpJars/agent.jar
     java -jar agent.jar \
       -url "http://localhost:18080" \
       -secret "d583f7de9fae...실제값..." \
       -name "mac-ui-tester" \
       -workDir "${HOME}/jenkins-agent"
```

이 명령을 **에이전트 머신 위에서** 그대로 실행한다 (시크릿은 매 실행마다 다르므로 setup.sh 출력에서 복사). `localhost` 는 에이전트가 컨트롤러와 같은 호스트일 때만 유효하고, 다른 머신이면 `localhost` → 컨트롤러의 IP/호스트명 으로 교체.

**완료 확인:**
1. Jenkins UI → `Jenkins 관리` → `노드 관리(Manage Nodes)` → `mac-ui-tester`
2. 상태가 **"Connected"** (또는 "Idle") 로 표시되면 OK

**자세한 단계별 안내**: [§8.6 Step 2 ~ Step 3](#step-2-agentjar-다운로드-및-에이전트-실행)
