...
| 용도 | 모델 | 크기 | 역할 |
| --- | --- | --- | --- |
| Planner + Healer **기본값** | **`qwen3.5:4b`** | ~3.4 GB | 자연어 → 9대 DSL JSON 생성, DOM 분석 → 대체 셀렉터 JSON 제안. Qwen 시리즈는 Gemma 대비 구조화 출력 fidelity 가 안정적이고, 4B 체급 중에서는 호스트 자원 부담이 가장 가벼워 기본값으로 선정 |
