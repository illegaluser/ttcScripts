...
**왜?** 작업 1 이 끝나도 "공급자(Ollama 자체)" 만 등록됐을 뿐이다. 어느 구체적 모델(`gemma4:e2b` 등)을 쓸지를 별도로 지정해야 Chatflow 가 동작한다.
...
| --- | --- | --- |
| Model Type | `LLM` | `LLM` |
| **Model Name** | `gemma4:e2b` ⚠️ 정확히 | `gemma4:e2b` ⚠️ 정확히 |
| **Base URL** | `http://host.docker.internal:11434` | `http://ollama:11434` |

4. `Save` 클릭 → 모델 목록에 표시되면 OK

> ⚠️ **Model Name 은 [dify-chatflow.yaml](dify-chatflow.yaml) 의 값과 바이트 단위로 정확히 일치**해야 한다. 파일에 `gemma4:e2b` 로 하드코딩되어 있다. 다른 모델을 쓰려면 DSL 파일도 함께 수정 후 Chatflow 를 재import.
>
...
| 용도 | 모델 | 크기 | 역할 |
| --- | --- | --- | --- |
| Planner + Healer **기본값** | **`gemma4:e2b`** | ~7.1 GB | 자연어 → 9대 DSL JSON 생성, DOM 분석 → 대체 셀렉터 JSON 제안. **8GB VRAM GPU (RTX 4070 Laptop 등) 에 딱 맞음**, 추론 속도 양호 |
| 대안/품질 우선 | `gemma4:e4b` | ~9.6 GB | 품질 더 좋지만 8GB VRAM 초과 → CPU/GPU 분할 로드로 **추론 매우 느려** Planner timeout 빈발. 16GB+ VRAM 서버 환경 권장 |
...
> Pull 한 뒤 [dify-chatflow.yaml](dify-chatflow.yaml) 의 `gemma4:e2b` 참조 2곳을 교체하고 Dify 에서 Chatflow 를 재import 한다.
...
```bash
docker exec -it e2e-ollama ollama pull gemma4:e2b
...
플러그인을 설치했다고 모델이 자동으로 잡히는 건 아니다. Ollama 공급자에 구체적 모델(`gemma4:e2b` 등)을 등록해야 Chatflow 에서 사용 가능하다.
...
| --- | --- | --- |
| **Model Type** | `LLM` | `LLM` |
| **Model Name** | `gemma4:e2b` | `gemma4:e2b` |
| **Base URL** | `http://host.docker.internal:11434` | `http://ollama:11434` |
...
**Step 4.** `Save` 클릭 → 에러 없이 저장되면 목록에 `gemma4:e2b` 가 `Ollama` 아래 등록된다.
...
>
> **함정 1 — Model Name 불일치**
> 위에서 입력한 Model Name 은 반드시 [dify-chatflow.yaml](dify-chatflow.yaml) 의 Planner/Healer LLM 노드 `name` 값과 **바이트 단위로 일치**해야 한다. 파일 안에 `gemma4:e2b` 로 하드코딩되어 있다. 다른 모델을 쓰려면 DSL 파일의 두 곳(Planner, Healer)을 바꾸고 Chatflow 를 **재import** 해야 한다. 불일치 시 Chatflow 실행에서 `model not found` 에러가 난다.
>
...
**모델 선택:** `gemma4:e2b` (§6.2 에서 Dify 공급자로 등록한 모델)
- Temperature: `0.3`
- 권장 대안: Pull 되어 있다면 `qwen3-coder:14b`, `qwen3-coder:30b` (4B 대비 JSON 품질↑)
- **주의**: [dify-chatflow.yaml](dify-chatflow.yaml) 은 이 섹션을 **DSL 파일로 자동 import** 하는 경로를 제공한다. 수동 import (DSL 파일에서 가져오기) 를 쓰면 §7.2~§7.6 의 노드 설정을 수동으로 재현할 필요 없이 한 번에 구성되고, 모델명은 파일 안에 `gemma4:e2b` 로 하드코딩되어 있다.
...
**모델 선택:** `gemma4:e2b` (Planner 와 동일 모델 공유, 단일 공급자 설정으로 충분)
...
> dify-chatflow.yaml 은 `gemma4:e2b` 로 하드코딩되어 있으므로 Dify 모델 공급자 등록 이름과 Ollama 태그 이름이 모두 정확히 `gemma4:e2b` 여야 한다.
