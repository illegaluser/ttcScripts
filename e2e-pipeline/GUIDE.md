...
git 저장소 전체를 clone 하거나 `e2e-pipeline/` 폴더만 복사해서 사용한다.

### 0.1 어디에 둘 것인가 (⚠️ 중요)

프로젝트를 두는 위치가 **성능과 안정성에 직접 영향**을 준다. 특히 Windows + Docker Desktop 사용자는 아래 선택이 docker build 체감 속도를 2~5배까지 바꾼다.

| 위치 | docker build / git | OneDrive 동기화 | 권장 용도 |
|---|---|---|---|
| **✅ WSL2 네이티브** — `~/projects/ttcScripts` (= `/home/<user>/projects/ttcScripts`) | **최고속** (ext4 직접 접근) | 없음 | **일상 개발 / 반복 설치 / 디버깅** (권장) |
| **⚠️ Windows 드라이브 마운트** — `/mnt/c/Users/.../ttcScripts` | 느림 (2~5배, 9P 프로토콜 경유) | Windows 쪽에서 OneDrive 자동 | 1회성 체험 / 비밀 변경 없이 한번만 돌려보는 경우 |
| **❌ Docker Desktop 내부 경로** — `/mnt/wsl/docker-desktop-bind-mounts/...` | — | — | **절대 사용 금지** (아래 경고 박스 참조) |

> ### ❌ 절대 가지 말 것: `/mnt/wsl/docker-desktop-bind-mounts/`
>
> 이 경로(예: `/mnt/wsl/docker-desktop-bind-mounts/Ubuntu/5929fcbd...`) 는 **Docker Desktop 이 내부 관리용으로 사용하는 WSL2 bind mount 저장소**다. 64자 hex 문자열은 Docker 의 content-addressable ID.
>
> - Docker 가 언제든 정리·재구성할 수 있어 사용자 파일이 유실될 수 있음
> - 사용자 편집용이 아님 (권한/inode 관리 Docker 소유)
> - 여기 있는 파일로 `setup.sh` 를 돌리면 스택 재빌드 시 불가사의한 에러 발생
>
> 만약 실수로 여기 들어왔다면 **즉시 `~/projects/` 같은 정상 경로로 이동**.
...
# 이후 모든 작업은 이 경로에서
cd e2e-pipeline
pwd    # /home/<user>/projects/ttcScripts/e2e-pipeline   ← 이런 형태여야 OK
```

**Windows 탐색기에서 접근하고 싶다면:** 주소창에 `\\wsl$\Ubuntu\home\<사용자명>\projects\ttcScripts` 입력 → 즐겨찾기 등록. 파일 탐색/편집 가능.

### 0.3 macOS / Linux 네이티브
...
### 0.4 Windows 네이티브 (Git Bash)

WSL2 를 쓰지 않고 Git Bash 에서 직접 돌리는 경우 (비권장, 하지만 가능):
...
> Git Bash 는 `setup.sh` 의 `detect_os()` 가 `MINGW*/MSYS*/CYGWIN*` 을 `windows` 로 감지해 기술적으로 동작하지만, WSL2 대비 Windows 경로 변환 / 9P 마운트 / 볼륨 bind mount 경합 이슈가 간헐적으로 발생한다. **WSL2 경로를 먼저 시도해보고 문제가 없으면 그대로 쓰길 권장.**

### 0.5 이후 모든 `docker compose` 명령

`e2e-pipeline/` 폴더 **안에서** 실행한다. 다른 위치에서 실행하면 `docker-compose.yaml` 을 찾을 수 없다.
...
> **Windows 사용자:** `setup.sh` 는 bash 스크립트이므로 **WSL2 Ubuntu 셸** 에서 실행하는 것을 권장한다 (성능 + Docker Desktop WSL2 backend 와의 매끄러운 통합). Git Bash 도 기술적으로는 동작하지만 Windows 경로 변환 / 9P 성능 / bind mount 경합 이슈가 간헐 발생한다 — 문제가 나면 WSL2 로 이전. **PowerShell / cmd 는 불가** (bash 인터프리터 없음).
>
> WSL2 에서 Docker 통합이 활성화되어 있어야 한다:
> `Docker Desktop → Settings → Resources → WSL Integration → 사용 중인 distro 체크.`
>
> 또한 프로젝트는 반드시 **WSL2 네이티브 경로** (`~/projects/ttcScripts/e2e-pipeline`) 에 둘 것. `/mnt/c/...` 에서도 동작은 하지만 2~5배 느리다. 자세한 내용은 [§0.1 어디에 둘 것인가](#01-어디에-둘-것인가-️-중요) 참조.
...
```bash
cd ~/projects/ttcScripts/e2e-pipeline
cp .env.example .env
...
>
> `AccessDeniedException` 이 나는 대표적 원인은 루트 소유 디렉터리(`/home/jenkins-agent`, `/var/lib/jenkins` 등)를 지정하는 것이다. 실행 셸에 따라 아래 값으로 교체:
>
> | 실행 환경 | `-workDir` 값 | 실제 확장 경로 예시 |
> |---|---|---|
> | **WSL2 Ubuntu** | `"$HOME/jenkins-agent"` | `/home/dscore-ttc/jenkins-agent` |
> | **macOS / Linux bash** | `"$HOME/jenkins-agent"` | `/Users/alice/jenkins-agent` 또는 `/home/alice/jenkins-agent` |
> | **Windows Git Bash** | `"$HOME/jenkins-agent"` | `/c/Users/alice/jenkins-agent` |
> | **Windows PowerShell** | `"$env:USERPROFILE\jenkins-agent"` | `C:\Users\alice\jenkins-agent` |
> | **Windows cmd** | `"%USERPROFILE%\jenkins-agent"` | `C:\Users\alice\jenkins-agent` |
>
> 공통 원칙:
> - 사용자 홈 디렉터리 하위에 두면 항상 안전.
> - 디렉터리가 없으면 agent.jar 가 자동 생성한다 (부모 경로에 쓰기 권한만 있으면 OK).
> - `/home/jenkins-agent` 처럼 루트만 쓸 수 있는 최상위 경로는 **금지**.
> - `C:\jenkins-agent` 같이 Administrator 권한이 필요한 경로도 **금지**.
...
### 프로젝트를 잘못된 경로에 두고 작업 중

**증상:**
- `docker build` 가 비정상적으로 느림 (10분 이상, 일반적으로는 2~3분)
- `git status` 가 수초~수십 초 걸림
- `find` 가 대기 상태에 빠짐
- setup.sh Phase 1 의 이미지 빌드가 터무니없이 오래 걸림
- 또는 **setup.sh 가 `/mnt/wsl/docker-desktop-bind-mounts/...` 경로에서 실행되고 있음**

**원인 — 두 가지 가능성:**

#### A. WSL2 에서 `/mnt/c/` 경로 사용 중 (느림, 하지만 동작함)

Windows 드라이브를 WSL2 가 9P 프로토콜로 마운트하는 구조라 네이티브 ext4 대비 **2~5배 느리다**. 기능상 문제는 없지만 반복 설치/디버깅엔 부적합.

**확인:**
```bash
pwd
# /mnt/c/Users/... 로 시작하면 이 경우
```

**해결:** [§0.1 어디에 둘 것인가](#01-어디에-둘-것인가-️-중요) 의 권장 절차대로 `~/projects/ttcScripts` 로 이전.
...
git clone https://github.com/illegaluser/ttcScripts.git
cd ttcScripts
git checkout feat/standalone-e2e-pipeline

# 기존 /mnt/c 의 스택은 완전히 정리
cd /mnt/c/Users/<사용자>/OneDrive/문서/dscore-ttc/ttcScripts/e2e-pipeline
docker compose down -v
...
#### B. `/mnt/wsl/docker-desktop-bind-mounts/...` 경로에 있음 (위험, 즉시 이동 필요)

**확인:**
```bash
pwd
# /mnt/wsl/docker-desktop-bind-mounts/Ubuntu/<64자 hex>/... 형태면 여기
```

**이 경로가 뭔가?** Docker Desktop 이 내부적으로 WSL2 bind mount 를 노출하는 **관리용 공간**이다. 64자 hex 문자열은 Docker 의 content-addressable ID. **사용자 편집용이 아니다** — Docker 가 언제든 정리·재구성할 수 있어 파일이 유실될 수 있고, 볼륨 마운트 경합이 발생할 수 있다.

**왜 여기 들어갔나?** 보통 사용자가 Docker 볼륨의 실제 내용을 탐색하려다 실수로 cd 한 경우다. 혹은 `docker inspect` 에서 본 경로를 그대로 따라간 경우.

**해결: 즉시 나와서 A 의 권장 경로로 git clone 한다.** 이 경로에 있는 파일은 복사하지 말고 버린다 (git push 된 상태라면 remote 에 모든 게 있으므로 손실 없음).
...
setup.sh Phase 0 에 이 경로 감지 로직이 있어서 여기서 실행하면 즉시 에러로 중단된다. 위험을 차단하기 위한 방어장치.
