...
| 용도 | 모델 | 크기 | 역할 |
| --- | --- | --- | --- |
| Planner + Healer **기본값** | **`gemma4:e4b`** | ~9.6 GB | 자연어 → 9대 DSL JSON 생성, DOM 분석 → 대체 셀렉터 JSON 제안. e4b (effective 4B) 는 품질 우선, 16GB+ RAM 권장 |
| 대안/경량 | `gemma4:e2b` | ~7.1 GB | 저자원 호스트용 (8GB RAM 노트북 등). 품질은 e4b 대비 약간 낮음 |
| 대안/실험용 | `qwen3.5:4b`, `gemma3:4b` | ~3.3~3.4 GB | 품질 비교용. Chatflow LLM 노드 `name` 만 교체 |
