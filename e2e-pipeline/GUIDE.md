...
이 문서는 `e2e-pipeline/` 독립 스택을 **docker-compose 기반 다중 컨테이너 방식**으로 처음부터 끝까지 구축하는 가이드다.
Jenkins + Dify + Ollama(LLM) + Playwright가 모두 포함된 **완전 독립 환경**을 구성한다.
기존 `docker-compose.yaml`(메인 DevOps 스택)과 **완전히 분리**되어 공존한다.

> **다른 배포 형태**: 단일 Docker 이미지로 폐쇄망에 옮겨 실행하는 **오프라인 올인원** 경로가 따로 있다. Mac (호스트 Ollama + 호스트 agent 하이브리드) 과 Windows 11 (Windows 네이티브 Ollama + WSL2 Ubuntu agent + WSLg) 을 지원한다. → [**e2e-pipeline/offline/README.md**](offline/README.md)
...
4. [**§9 E2E 테스트 실행**](#9-e2e-테스트-실행) — 첫 빌드 (에이전트는 setup.sh 가 자동 기동)

> **오프라인 단일 이미지 경로를 원한다면 이 가이드가 아닌 [offline/README.md](offline/README.md) 를 따르라** — build-allinone.sh 로 tar.gz 를 말아 폐쇄망에 옮긴 뒤 `docker run` + `wsl-agent-setup.sh` 만으로 구축이 끝난다.
...
| Java 21 | Jenkins 에이전트(agent.jar) 실행에 필요 — 컨트롤러가 jdk21 이미지라 remoting class file v65 를 보내므로 에이전트도 Java 21 필수. macOS 에서 `brew install openjdk@21` + keg-only PATH 연결 + JVM 심링크는 setup.sh 가 자동 처리 |
...
> **Fresh macOS 대응 범위**: Homebrew · python3 · openjdk@21(PATH + JVM 심링크) · Ollama · `OLLAMA_HOST=0.0.0.0` 바인딩(launchctl setenv + brew services restart/Ollama.app 재시작)은 모두 자동 설치/설정된다. Docker Desktop 만 사전 설치가 필요하다 (GUI 라이선스 동의 때문). 자세한 자동화 범위는 [§자동 설치](#자동-설치-setupsh) 의 Phase 표 참조.
...
| `OLLAMA_MODEL` | `gemma4:e4b` | Phase 3 에서 확인/Pull 할 모델명. dify-chatflow.yaml 과 일치 필수 |
...
| `SCENARIO_TIMEOUT_SEC` | `300` | Dify Planner 시나리오 생성 timeout(초). retry 1회. `gemma4:e4b` 등 느린 모델의 첫 토큰 지연 허용 |
...
| 4-3a | Ollama 플러그인 자동 설치 (marketplace `latest_package_identifier` 조회 → `/plugin/install/marketplace` POST → `/plugin/list` 폴링) | ✅ 자동 (실패 시 수동 fallback 안내) |
| 4-3b | Ollama 모델 공급자에 `OLLAMA_MODEL` 등록 (`/model-providers/langgenius/ollama/ollama/models/credentials` POST) | ✅ 자동 (실패 시 수동 fallback 안내) |
| 4-3.5 | 동일 이름 기존 Chatflow App 감지 시 DELETE (재실행 idempotent 보장) | ✅ 자동 |
...
| 5-2 | Jenkins Credentials 등록/업데이트 (`dify-qa-api-token`) — create 실패 시 `config.xml` 로 덮어쓰기 폴백 | ✅ 자동 (API Key 획득된 경우) |
...
| 6-1 | Java 21 설치 확인/자동 설치 (apt/brew/winget). 컨트롤러 이미지가 jdk21 이라 에이전트도 Java 21 필수. macOS 는 설치 후 `ensure_java_on_path` 가 keg-only PATH prepend + `/Library/Java/JavaVirtualMachines` 심링크까지 자동 생성 (sudo 비밀번호 프롬프트가 뜰 수 있음) | ✅ 자동 |
...
| **Phase 7** | **에이전트 자동 기동** | |
| 7-1 | 노드 시크릿 추출 + `agent.jar` 백그라운드 실행 (`nohup`) | ✅ 자동 |
| 7-2 | 에이전트 연결 확인 (Jenkins API 로 online 여부 폴링) | ✅ 자동 |

### ⚠️ setup.sh 가 끝난 뒤 반드시 해야 할 수동 작업

> setup.sh 가 `[✓] DSCORE Zero-Touch QA 스택 설치 완료` 를 찍었다고 모든 게 끝난 게 아니다.
> Phase 7 에서 에이전트까지 자동으로 기동하지만,
> **아래 작업은 여전히 수동으로 필요**하다 — Dify 마켓플레이스 UI 가 API 자동화를 지원하지 않기 때문이다.
>
> **이 작업들을 하지 않으면 빌드가 절대 동작하지 않는다.** 빠뜨리면 `model not found` 로 막힌다.
...
**왜?** 작업 1 이 끝나도 "공급자(Ollama 자체)" 만 등록됐을 뿐이다. 어느 구체적 모델(`gemma4:e4b` 등)을 쓸지를 별도로 지정해야 Chatflow 가 동작한다.
...
| --- | --- | --- |
| Model Type | `LLM` | `LLM` |
| **Model Name** | `gemma4:e4b` ⚠️ 정확히 | `gemma4:e4b` ⚠️ 정확히 |
| **Base URL** | `http://host.docker.internal:11434` | `http://ollama:11434` |

4. `Save` 클릭 → 모델 목록에 표시되면 OK

> ⚠️ **Model Name 은 [dify-chatflow.yaml](dify-chatflow.yaml) 의 값과 바이트 단위로 정확히 일치**해야 한다. 파일에 `gemma4:e4b` 로 하드코딩되어 있다. 다른 모델을 쓰려면 DSL 파일도 함께 수정 후 Chatflow 를 재import.
>
...
#### ✅ 작업 3 — Jenkins 에이전트 (자동 기동)

setup.sh **Phase 7** 에서 `agent.jar` 를 `nohup` 으로 자동 백그라운드 기동한다. 정상적으로 완료되면 배너에 `✓ mac-ui-tester 에이전트: 연결됨` 이 표시된다.

**자동 기동 실패 시 수동 실행:**

setup.sh 종료 배너에 수동 명령이 표시된다. 그대로 복사해 실행:
...
> 에이전트 로그 확인: `tail -f $HOME/jenkins-agent/agent.log`
>
> 에이전트 프로세스 재시작: `pkill -f 'agent.jar.*mac-ui-tester'` 후 위 명령 실행.

**자세한 단계별 안내**: [§8.6](#86-mac-ui-tester-에이전트-등록) (수동 설치, systemd 서비스, launchd 등)

---

#### 작업 완료 후 첫 빌드 실행

위 작업이 모두 끝났다면 시스템이 동작 가능한 상태다. 빠른 동작 확인:
...
- `agent offline` → `tail -f $HOME/jenkins-agent/agent.log` 로 에이전트 상태 확인
...
| 용도 | 모델 | 크기 | 역할 |
| --- | --- | --- | --- |
| Planner + Healer **기본값** | **`gemma4:e4b`** | ~9.6 GB | 자연어 → 9대 DSL JSON 생성, DOM 분석 → 대체 셀렉터 JSON 제안. 품질 우선 모델. 8GB VRAM 호스트에서는 CPU/GPU 분할 로드로 느려질 수 있음 — `<think>` 금지 프롬프트 + `max_tokens` 축소(Planner 2048/Healer 1024) + Planner timeout 300s 로 완화 |
| 경량 대안 | `gemma4:e2b` | ~7.1 GB | 8GB VRAM 완전 수용, 속도 우선 시 사용. 품질은 e4b 대비 약간 낮음 |
| 저자원 대안 | `qwen3.5:4b`, `gemma3:4b` | ~3.3~3.4 GB | 구성 검증 및 품질 비교용 |
...
> Pull 한 뒤 [dify-chatflow.yaml](dify-chatflow.yaml) 의 `gemma4:e4b` 참조 2곳을 교체하고 Dify 에서 Chatflow 를 재import 한다.
...
> WSL2 + Windows Ollama GUI 조합에서 Linux ollama 를 같이 깔면 두 인스턴스가 각자 다른 모델 저장소 + GPU/RAM 경쟁으로 빌드 디버깅이 매우 혼란해집니다 (실제 사고 사례: 한 쪽은 `gemma4:e4b` 있고 다른 쪽은 없어서 Dify 호출만 실패하는데 WSL CLI 테스트는 성공하는 현상).
...
```bash
docker exec -it e2e-ollama ollama pull gemma4:e4b
...
> **✅ 자동화됨 (setup.sh Phase 4-3a / 4-3b).**
> setup.sh 가 다음을 순차 처리한다:
> 1. `marketplace.dify.ai/api/v1/plugins/batch` 로 `langgenius/ollama` 의 `latest_package_identifier` 조회
> 2. `POST /console/api/workspaces/current/plugin/install/marketplace` 호출
> 3. `/plugin/list` 폴링하여 설치 완료 확인 (최대 90초)
> 4. `POST /model-providers/langgenius/ollama/ollama/models/credentials` 로 `OLLAMA_MODEL` 자동 등록
>
> **수동 절차가 필요한 경우 (모두 해당해야 함):**
> - 폐쇄망 환경이라 `marketplace.dify.ai` 차단됨
> - 또는 setup.sh 가 자동화 실패를 경고했음 (`⚠️ Dify Ollama 자동 설정 일부 실패` 배너)
>
> 그 외 일반 환경에서는 아래 단계를 건너뛰어도 된다.
...
플러그인을 설치했다고 모델이 자동으로 잡히는 건 아니다. Ollama 공급자에 구체적 모델(`gemma4:e4b` 등)을 등록해야 Chatflow 에서 사용 가능하다.
...
| --- | --- | --- |
| **Model Type** | `LLM` | `LLM` |
| **Model Name** | `gemma4:e4b` | `gemma4:e4b` |
| **Base URL** | `http://host.docker.internal:11434` | `http://ollama:11434` |
...
**Step 4.** `Save` 클릭 → 에러 없이 저장되면 목록에 `gemma4:e4b` 가 `Ollama` 아래 등록된다.
...
>
> **함정 1 — Model Name 불일치**
> 위에서 입력한 Model Name 은 반드시 [dify-chatflow.yaml](dify-chatflow.yaml) 의 Planner/Healer LLM 노드 `name` 값과 **바이트 단위로 일치**해야 한다. 파일 안에 `gemma4:e4b` 로 하드코딩되어 있다. 다른 모델을 쓰려면 DSL 파일의 두 곳(Planner, Healer)을 바꾸고 Chatflow 를 **재import** 해야 한다. 불일치 시 Chatflow 실행에서 `model not found` 에러가 난다.
>
...
**모델 선택:** `gemma4:e4b` (§6.2 에서 Dify 공급자로 등록한 모델)
- Temperature: `0.3`
- 권장 대안: Pull 되어 있다면 `qwen3-coder:14b`, `qwen3-coder:30b` (4B 대비 JSON 품질↑)
- **주의**: [dify-chatflow.yaml](dify-chatflow.yaml) 은 이 섹션을 **DSL 파일로 자동 import** 하는 경로를 제공한다. 수동 import (DSL 파일에서 가져오기) 를 쓰면 §7.2~§7.6 의 노드 설정을 수동으로 재현할 필요 없이 한 번에 구성되고, 모델명은 파일 안에 `gemma4:e4b` 로 하드코딩되어 있다.
...
**모델 선택:** `gemma4:e4b` (Planner 와 동일 모델 공유, 단일 공급자 설정으로 충분)
...
> - §8.6 mac-ui-tester 노드 → **✅ 자동 등록 + 에이전트 자동 기동** (Phase 7)
>
> **수동으로 docker compose 를 기동한 경우:**
> - §8.1 부터 순서대로 모두 진행
>
> setup.sh Phase 7 이 에이전트를 `nohup` 으로 자동 기동한다. 실패 시에만 §8.6 의 수동 절차를 따른다.
...
> **✅ setup.sh 가 API Key 까지 뽑아냈다면 이미 완료.** Phase 5-2 에서 `dify-qa-api-token` 이라는 id 로 자동 등록된다.
> 기존 id 가 존재해도 `createCredentials` 실패 시 `config.xml` POST 로 **자동 덮어쓰기** 되므로, 재실행마다 Dify 가 재생성한 최신 API Key 가 반영된다.
> 확인 방법: `Jenkins 관리` → `Credentials` → `System` → `Global credentials` → 목록에 `dify-qa-api-token` 이 보이면 OK.
>
> **⚠️ setup.sh 가 `Credentials 등록/업데이트 실패` 를 경고했다면** (또는 API Key 가 없으면) 아래 수동 절차 필요.
...
> ✅ **setup.sh 사용자는 이 단계가 자동 처리된다.** Phase 6-1 이 `brew install openjdk@21` 실행 후 `ensure_java_on_path` 헬퍼로 keg-only PATH prepend 와 `/Library/Java/JavaVirtualMachines/openjdk-21.jdk` 심링크(sudo 프롬프트 포함)를 자동 생성한다. Homebrew 가 없으면 같은 Phase 에서 공식 스크립트로 자동 설치된다.
...
2. **Temurin 21 (LTS)** 선택 → 본인 OS 에 맞는 installer (`.msi`) 다운로드
3. 설치 관리자 실행 → **"Set JAVA_HOME variable"** 과 **"Add to PATH"** 옵션 체크
4. 설치 완료 후 새 터미널(PowerShell/cmd)에서 `java -version` 확인
</details>

> **왜 Java 21?** Jenkins 컨트롤러 이미지가 `jenkins/jenkins:lts-jdk21` 이라 remoting 이 Java 21 class file (v65) 을 에이전트에 전송한다. Java 17 에이전트는 `UnsupportedClassVersionError` 로 거부되므로 호스트도 Java 21 이상이 필수다.
...
**원인:** 에이전트 머신의 Java 버전이 너무 낮음. class file version 대응표: 52 = Java 8, 61 = Java 17, 65 = Java 21. 컨트롤러가 jdk21 이미지면 remoting 이 v65 클래스를 보내므로 에이전트가 Java 17 이하이면 이 에러가 난다.

**해결:** §8.6.1 의 "사전 조건: JDK 21 설치" 를 따라 Java 21 설치. 설치 후 확인:

```bash
java -version
# openjdk version "21.x.x" 또는 이상
```
...
> dify-chatflow.yaml 은 `gemma4:e4b` 로 하드코딩되어 있으므로 Dify 모델 공급자 등록 이름과 Ollama 태그 이름이 모두 정확히 `gemma4:e4b` 여야 한다.
...
### Jenkins 빌드 120초 Read timeout (Dify `/v1/chat-messages`) — Ollama 플러그인/모델 미등록

**증상:** 빌드 로그에 `Read timed out (read timeout=120)` × 3회 재시도 후 실패. setup.sh 는 정상 종료(Phase 4-6 API Key 발급, Phase 5-2 Credentials 등록까지 모두 ✅).

**원인:** 직전에 `docker compose down -v` 가 실행되어 `e2e-dify-db` (Postgres) + `e2e-dify-plugin` 볼륨이 비워졌다. 새로 기동된 Dify 에는:
- Ollama 플러그인이 설치돼 있지 않음
- `OLLAMA_MODEL` 모델 공급자 등록도 없음

이 상태로 Chatflow 의 LLM 노드가 Ollama 모델을 호출하면 plugin daemon 으로 라우팅이 막혀 무응답 → 클라이언트 측에서 Read timeout 으로 보인다.

**해결 (자동):** 최신 setup.sh 의 Phase 4-3a / 4-3b 가 marketplace 호출 + 모델 등록을 자동 수행한다. 재실행으로 복구.

**해결 (수동, 폐쇄망 등 마켓플레이스 차단 시):** §6.2 의 단계별 수동 절차.

**검증:**
```bash
# 플러그인 설치 여부
curl -s -b .dify-cookies.txt -H "X-CSRF-Token: $TOKEN" \
  http://localhost:18081/console/api/workspaces/current/plugin/list \
  | python3 -m json.tool | grep langgenius/ollama

# 모델 등록 여부
curl -s -b .dify-cookies.txt -H "X-CSRF-Token: $TOKEN" \
  http://localhost:18081/console/api/workspaces/current/model-providers/langgenius/ollama/ollama/models \
  | python3 -m json.tool | grep -A1 '"model"'
```

---

### Jenkins 빌드 401 UNAUTHORIZED (Dify `/v1/chat-messages`) — stale API Key

**증상:** 빌드 로그에 `Dify API 통신 실패: 401 Client Error: UNAUTHORIZED for url: .../v1/chat-messages`. setup.sh 로그에는 Phase 4-6 API Key 발급 성공, Phase 5-2 는 `Credentials 등록 실패` HTTP 409.

**원인:** Jenkins 에 이전 실행에서 등록된 `dify-qa-api-token` 이 남아 있는데, Dify 는 재기동·재설정 과정에서 App 을 새로 만들거나 API Key 를 새로 발급한다. 옛 setup.sh 는 `POST /createCredentials` 만 호출했고 409 Conflict 시 **덮어쓰지 않고 warn 만 찍었다** → Jenkins 는 stale 한 옛 키를 그대로 사용 → 401.

**해결 (자동):** 최신 setup.sh 는 409/기타 실패 시 `POST /credentials/store/system/domain/_/credential/dify-qa-api-token/config.xml` 로 **자동 덮어쓰기 폴백** 한다. 재실행하면 정상화.

**해결 (수동):** `Jenkins 관리 → Credentials → System → Global → dify-qa-api-token → Update` 에서 setup.sh 로그의 `Dify API Key 발급 완료: app-xxxx` 값을 그대로 붙여넣고 저장.

**부가 원인 — Dify 쪽 중복 App:** 재실행 시 동일 이름의 Chatflow App 이 여러 개 생기면 어떤 App 의 API Key 가 발급됐는지가 흔들린다. 최신 setup.sh 는 Phase 4-3.5 에서 동일 이름 기존 App 을 전부 DELETE 한 뒤 import 한다.
