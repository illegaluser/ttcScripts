...
class LocalHealer:
    ...
    SELECTOR_MAP = {
        # [변경] fill/press 셀렉터에 [role='combobox'], input[type='search'],
        #        input[type='text'] 추가
        # 원본: input, textarea, [role='textbox'], [role='searchbox'], [contenteditable='true']
        # 사유: Google 검색창은 실제 DOM 에서 role="combobox" + type="search" 로 렌더링된다.
        #       LLM 이 role="searchbox" 로 타겟을 생성하더라도 LocatorResolver 가 먼저
        #       실패했을 때 LocalHealer 가 combobox 와 type=search 입력필드를 커버해야
        #       self-healing 이 동작한다.
        "fill": (
            "input, textarea, [role='textbox'], [role='searchbox'], "
            "[role='combobox'], input[type='search'], input[type='text'], "
