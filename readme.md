...
이 문서는 **폐쇄망 환경에서 단일 로컬 머신으로 구동 가능한, 로컬 LLM 기반 온프레미스 QAOps 시스템**을 처음부터 끝까지 구축하는 완전한 가이드다. 모든 서비스는 Docker 컨테이너로 로컬에서 실행되며, AI 추론은 Ollama를 통해 로컬 GPU/CPU에서 수행된다. 외부 인터넷 연결 없이 전체 파이프라인이 동작한다.
...
1. 문서/코드/웹을 Dify Knowledge Base에 자동 업로드
...
3. **Job #3: DSCORE-Code-Knowledge-Sync** - 코드 컨텍스트 지식화
4. **Job #4: DSCORE-Quality-Issue-Workflow** - 정적 분석 및 이슈 자동 등록
5. **Job #5: DSCORE-Web-Knowledge-Sync** - 웹 콘텐츠 수집 및 지식화
6. **Job #6: DSCORE-ZeroTouch-QA** - Dify Brain 기반 자율 E2E 테스트 (v3.3, Mac Agent)
7. **Job #7: DSCORE-Agent-Eval** - 외부 AI 에이전트 11대 지표 자동 평가
...
4. **Jenkins → Ollama:** `http://host.docker.internal:11434`
...
| --- | --- | --- |
| `gitlab-access-token` | GitLab PAT (`api`, `read_repository`, `write_repository` 권한) | Section 6 (Job #1-5) |
| `sonarqube-token` | SonarQube User Token (Security > Generate Tokens) | Section 6 (Job #4) |
| `dify-knowledge-key` | Dify 문서형 API Key | Section 6 (Job #1-3, 5) |
| `dify-dataset-id` | Dify 문서형 Dataset UUID | Section 6 (Job #1-3, 5) |
| `dify-knowledge-key-qa` | Dify Q&A형 API Key | Section 6 (Job #2) |
| `dify-dataset-id-qa` | Dify Q&A형 Dataset UUID | Section 6 (Job #2) |
| `dify-qa-api-token` | Dify Zero-Touch QA Chatflow API Key | Section 9 (Job #6) |
| `langfuse-public-key` | Langfuse Public Key (`pk-lf-...`) | Section 10 (Job #7) |
| `langfuse-secret-key` | Langfuse Secret Key (`sk-lf-...`) | Section 10 (Job #7) |
...
3. **Job #3** `repo_context_builder.py` — Git 저장소의 구조와 핵심 파일을 분석하여 코드 컨텍스트 문서를 생성하고 Dify에 업로드한다.
4. **Job #4** `sonar_issue_exporter.py` + `dify_sonar_issue_analyzer.py` + `gitlab_issue_creator.py` — SonarQube 정적 분석 결과를 LLM으로 진단하고 GitLab Issue로 자동 등록한다.
5. **Job #5** `domain_knowledge_builder.py` — 지정된 웹 페이지를 크롤링하여 텍스트를 추출하고 Dify Knowledge Base에 업로드한다.
...
### 4.2 `repo_context_builder.py` (코드 지식화)
...
### 4.3 `sonar_issue_exporter.py` (이슈 추출)
...
### 4.4 `dify_sonar_issue_analyzer.py` (LLM 분석)
...
### 4.5 `gitlab_issue_creator.py` (이슈 등록 및 중복 방지)
...
### 4.6 `domain_knowledge_builder.py` (웹 수집 및 정제)
...
* Job #1, #3, #5으로 업로드한 Dataset을 선택한다.
...
### 6.3 Job #3: DSCORE-Code-Knowledge-Sync (코드 컨텍스트)

**목적:**

Git 레포지토리의 디렉터리 구조와 주요 파일 내용을 추출하여 컨텍스트 문서를 생성하고, Dify Dataset에 업로드한다.
...
pipeline {
    ...
    post {
        success { echo "[Success] 코드 지식화 작업이 완료되었습니다." }
        failure { echo "[Failure] 작업 중 오류가 발생했습니다. 로그를 확인하세요." }
    }
}
```

### 6.4 Job #4: DSCORE-Quality-Issue-Workflow (정적 분석 자동화)
...
### 6.5 Job #5: DSCORE-Web-Knowledge-Sync (웹 콘텐츠 수집)
...
### 9.7 Jenkins Pipeline (Job #6: DSCORE-ZeroTouch-QA) 전체 스크립트
...
## 10. 외부 AI 에이전트 평가 시스템 (Job #7: DSCORE-Agent-Eval)
