...
| AI 평가 | DeepEval, Promptfoo, Langfuse | LLM 채점, 보안 검사, 관측성 |
| 테스트 | pytest, jsonschema | 자동화 테스트 실행 |
...
2. 환경변수 파일을 복사한다 (이 파일이 없으면 Dify가 기동되지 않는다).

```bash
cp .env.example .env
```

3. Dify의 api, worker, nginx 서비스를 `devops-net`에 연결하는 override 파일을 생성한다. 이 파일이 있어야 Jenkins 컨테이너에서 `http://api:5001`로 Dify를 호출할 수 있다.
...
5. Jenkins 컨테이너에서 Dify API로의 네트워크 연결을 확인한다.
...
| `eval_runner/openai_wrapper_api.py` | OpenAI API를 표준 형태로 래핑하는 경량 서버 |
| `eval_runner/gemini_wrapper_api.py` | Gemini API를 표준 형태로 래핑하는 경량 서버 (throttling 포함) |
...
웹사이트를 크롤링하여 최신 정보를 수집·정제하고, Dify 지식베이스에 Markdown 형태로 업로드하는 파이프라인이다. Crawl4AI로 페이지를 수집하고 BeautifulSoup로 본문만 추출한 뒤, `doc_processor.py`(Section 5.3 참고)를 통해 Dify에 업로드한다.

#### 유저플로우 (파이프라인 실행 흐름)

> `Stage 1` Local Document Conversion → `Stage 2` Web Scraping & Refinement → `Stage 3` Manual Approval → `Stage 4` Final Upload to Dify

| 단계 | Jenkins Stage 명 | 수행 내용 |
|---|---|---|
| 1 | Local Document Conversion | 기존 로컬 문서 변환 및 result 폴더 초기화 |
| 2 | Web Scraping & Refinement | Crawl4AI로 ROOT_URL 내부 링크 수집 및 본문 추출 |
...
| 스크립트 | 역할 |
|---|---|
| `domain_knowledge_builder.py` | 웹 크롤링(Crawl4AI) 및 HTML 정제(BeautifulSoup), URL/노이즈 제거 후 Markdown 저장 |
| `doc_processor.py` | 로컬 문서 변환(convert) 및 Dify Dataset 업로드(upload) |

#### 파이썬 스크립트: `domain_knowledge_builder.py` (웹 수집 및 정제)

루트 URL에서 내부 링크를 수집(Phase 1)한 뒤, 각 페이지를 방문하여 본문만 추출(Phase 2)하는 2단계 구조이다. 최대 50개 페이지로 제한하는 안전장치가 있다.
...
PDF, Word, Excel, PPTX 등 문서를 Markdown으로 변환한 뒤, Dify 지식베이스(Full-text 기반)에 업로드하는 파이프라인이다.

> `doc_processor.py`는 이 파이프라인 외에 5.2(웹스크래핑), 5.4(Q&A 지식주입), 5.5(코드 사전학습)에서도 공유하는 핵심 스크립트이다.
...
| 스크립트 | 역할 |
|---|---|
| `doc_processor.py convert` | PyMuPDF·python-docx·pandas·LibreOffice로 문서를 Markdown 변환 |
| `doc_processor.py upload` | 변환 결과를 Dify API로 업로드 (doc_form=text_model) |

#### 파이썬 스크립트: `doc_processor.py` (문서 변환 및 업로드)

이 스크립트는 두 가지 명령을 제공한다.

- **`convert`**: SOURCE_DIR의 문서(PDF, DOCX, XLSX, TXT, PPTX)를 Markdown으로 변환하여 RESULT_DIR에 저장한다.
- **`upload`**: RESULT_DIR의 결과물을 Dify Dataset에 업로드한다. Dify 문서 생성 API의 `doc_form`을 Dataset과 일치시키도록 강제한다.
...
문서를 Q&A 쌍으로 변환하여 Dify 지식베이스(Q&A 모델)에 업로드하는 파이프라인이다. 사전 생성된 Q&A가 있으면 AI가 유사 질문에 더 정확하게 답변할 수 있다.

> `doc_processor.py`는 Section 5.3에서 상세 기술한 동일 스크립트이다. 여기서는 `DIFY_DOC_FORM`을 `qa_model`로 설정하여 Q&A형 Dataset에 업로드한다.
...
| 스크립트 | 역할 |
|---|---|
| `doc_processor.py convert` | 원본 문서(PDF/DOCX/XLSX/PPTX)를 Markdown으로 변환 |
| `doc_processor.py upload` | 변환 결과를 Dify API로 업로드 (doc_form=qa_model, Q&A 쌍 생성) |
...
Git 저장소의 코드 구조와 주요 파일 내용을 분석하여 `context.md` 문서를 생성하고, Dify 지식베이스에 업로드하는 파이프라인이다. 이 문서를 학습한 AI는 프로젝트 구조와 코드를 이해한 상태로 답변할 수 있다.

> `doc_processor.py`는 Section 5.3에서 상세 기술한 동일 스크립트이다.
...
| 2 | Build Context | 디렉터리 트리 + 주요 파일 내용을 context.md로 생성 |
| 3 | Upload Context | context.md를 Dify 문서형 Dataset에 업로드 |

#### 스크립트별 기능요약

| 스크립트 | 역할 |
|---|---|
| `repo_context_builder.py` | Git 저장소 순회, 디렉터리 트리 및 주요 파일(README, package.json 등) 내용을 context.md로 통합 |
| `doc_processor.py upload` | 생성된 context.md를 Dify Dataset에 업로드 |

#### 파이썬 스크립트: `repo_context_builder.py` (코드 지식화)

Git 저장소를 순회하여 디렉터리 트리와 주요 파일(README, package.json 등)의 내용을 하나의 Markdown 문서로 합친다. `.git`, `node_modules`, `build` 등 불필요한 폴더는 자동 제외한다.
...
1. **이슈 추출** (`sonar_issue_exporter.py`): SonarQube API에서 이슈를 JSON으로 추출한다.
2. **LLM 분석** (`dify_sonar_issue_analyzer.py`): Dify Workflow를 통해 각 이슈를 LLM으로 진단하고, 원인/해결방안을 생성한다.
3. **이슈 등록** (`gitlab_issue_creator.py`): 진단 결과를 GitLab Issue로 자동 등록한다. 중복 방지 메커니즘이 적용된다.
...
| 단계 | Jenkins Stage 명 | 수행 내용 |
|---|---|---|
| 1 | Export Sonar Issues | SonarQube API에서 미해결 이슈를 JSON으로 추출 |
| 2 | Analyze by Dify Workflow | Dify LLM Workflow로 각 이슈의 원인/해결방안 생성 |
| 3 | Create GitLab Issues (Dedup) | 분석 결과를 GitLab Issue로 등록 (중복 방지 적용) |

#### 스크립트별 기능요약

| 스크립트 | 역할 |
|---|---|
| `sonar_issue_exporter.py` | SonarQube `/api/issues/search`에서 이슈를 페이징 수집, 룰 상세·코드 스니펫으로 enrichment 후 JSON 저장 |
| `dify_sonar_issue_analyzer.py` | Dify Workflow API를 호출하여 각 이슈를 LLM으로 진단 (원인·해결방안 생성, 3회 재시도) |
| `gitlab_issue_creator.py` | 진단 결과를 GitLab Issue로 자동 등록, search 기반 중복 검사 (sonar_issue_key 검색), form-encoded POST |
...
| 스크립트 | 역할 |
|---|---|
| `mac_local_executor.py` | 자율 QA 엔진 본체: Playwright 브라우저 제어, 7단계 시맨틱 DOM 탐색, 3단계 하이브리드 자가치유, Dify Brain 연동 |
...
Dify 콘솔에서 새 앱을 생성할 때 반드시 **Chatflow** 타입을 선택한다. `run_mode` 변수에 따라 Planner / Vision Refactor / Healer LLM 노드로 분기하는 구조이다. 각 노드에는 9대 DSL 스키마를 엄격히 준수하도록 시스템 프롬프트를 설정한다.

##### 5.8.5.1 전역 변수 (Start Node)
...
##### 5.8.5.3 Planner LLM 노드 (Flow 1, 2 처리용)
...
##### 5.8.5.4 Vision Refactor LLM 노드 (Flow 3 처리용)
...
class QAExecutor:
    ...
    def execute(self, scenario, is_ci):
        ...
        with sync_playwright() as p:
            # CI(Jenkins Mac Agent)에서는 headed 모드로 실행하여 화면 증적을 남기고,
...
Mac Local 에이전트(5.8.4)에서 `mac_local_executor.py`(5.8.6)를 실행하는 파이프라인이다. Python venv 캐싱, 환경 변수 자동 주입, 산출물 영구 보관이 포함되어 있다.
