...
1. **`DSCORE-TTC 지식주입 (문서 단순 청깅 및 임베딩)`**: PDF, Word 등 문서를 분석하여 AI 지식 베이스에 업로드합니다. (Full-text 기반)
2. **`DSCORE-TTC 지식주입 (질문 및 답변 사전생성)`**: 문서를 Q&A 쌍으로 변환하여 AI 답변 품질을 높입니다.
3. **`DSCORE-TTC 코드 사전학습`**: Git 저장소의 코드 구조와 내용을 분석하여 AI가 학습합니다.
4. **`DSCORE-TTC 웹스크래핑`**: 웹사이트를 크롤링하여 최신 정보를 지식 베이스에 추가합니다.
5. **`DSCORE-TTC 코드 정적분석`**: SonarQube를 통해 소스 코드의 품질 이슈(버그, 취약점)를 탐지합니다.
6. **`DSCORE-TTC 코드 정적분석 결과분석 및 이슈등록`**: 탐지된 품질 이슈를 AI가 분석하고 GitLab에 이슈로 자동 등록합니다.
7. **`DSCORE-ZeroTouch-QA`**: 자연어 시나리오 기반으로 웹 애플리케이션의 E2E 테스트를 자율적으로 수행합니다.
8. **`DSCORE-TTC AI평가`**: 외부 AI 에이전트의 성능을 11대 지표에 따라 정량적으로 자동 평가합니다.
...
각 Jenkins 파이프라인은 특정 목적을 위해 하나 이상의 파이썬 스크립트를 실행합니다.

-   **지식 주입 (문서):** `doc_processor.py` 스크립트를 사용하여 PDF, DOCX 등 다양한 형식의 문서를 Markdown으로 변환하고, 이를 일반 텍스트 또는 Q&A 형식으로 Dify 지식 베이스에 업로드합니다.
-   **지식 주입 (코드):** `repo_context_builder.py` 스크립트를 사용하여 Git 저장소의 구조와 핵심 파일 내용을 분석, 코드 컨텍스트 문서를 생성하고 Dify에 업로드합니다.
-   **지식 주입 (웹):** `domain_knowledge_builder.py` 스크립트를 사용하여 지정된 웹 페이지를 크롤링하고 Dify 지식 베이스에 업로드합니다.
-   **품질 분석 및 이슈 등록:** `sonar_issue_exporter.py`로 SonarQube 분석 결과를 추출하고, `dify_sonar_issue_analyzer.py`로 AI 진단을 수행한 뒤, `gitlab_issue_creator.py`를 통해 GitLab에 이슈를 자동으로 등록하는 3단계 흐름으로 동작합니다.
...
### 6.1 파이프라인: DSCORE-TTC 지식주입 (문서 단순 청깅 및 임베딩)
...
### 6.2 파이프라인: DSCORE-TTC 지식주입 (질문 및 답변 사전생성)
...
### 9.7 Jenkins Pipeline (`DSCORE-ZeroTouch-QA`) 전체 스크립트
...
## 10. 외부 AI 에이전트 평가 시스템 (`DSCORE-TTC AI평가`)
