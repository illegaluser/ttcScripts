...
- **파이프라인 단계**:
  ...
  3. **Target URL 연결 검증**: 평가 전에 Jenkins 컨테이너에서 `TARGET_URL` 접근 가능 여부를 먼저 확인합니다. 연결 불가 시 `HTTPConnectionPool ... [Errno 101] Network is unreachable` 유형의 원인을 즉시 표시하고 평가를 중단합니다.
