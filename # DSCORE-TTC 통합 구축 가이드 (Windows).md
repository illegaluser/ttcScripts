...
| 환경변수 | 설명 | 권장값 |
| --- | --- | --- |
| `OLLAMA_GPU_LAYERS` | 모델의 몇 개 레이어를 GPU VRAM에 올릴지 결정. `-1`이면 전체 모델을 GPU에 로드 | `-1` (VRAM 충분 시) |
| `OLLAMA_KEEP_ALIVE` | 모델을 VRAM에 유지하는 시간. 평가 중 모델 로드/언로드 반복을 방지 | `30m` |
| `OLLAMA_NUM_PARALLEL` | 동시 요청 처리 수. 멀티턴 평가 시 병렬 처리 가능 | `2` (VRAM 24GB 이상) |
...
# 기대 출력: qwen3-coder:30b 모델이 GPU에 로드되어 있어야 한다
# NAME              ID    SIZE   PROCESSOR    UNTIL
# qwen3-coder:30b   ...   18 GB  100% GPU     30 minutes from now
```
...
# Ollama 재시작
```
3. 더 작은 모델 사용을 고려한다 (예: `qwen3-coder:14b` → 약 9 GB VRAM).
