...
> **이 문서는 `readme.md`(macOS/Linux 기준)의 Windows 11 환경 대응판이다.**
> Docker 컨테이너 내부에서 실행되는 코드(Python 스크립트, Jenkinsfile, docker-compose.yaml, Dockerfile 등)는 OS와 무관하게 동일하므로, macOS 가이드와 공유한다.
> Section 1(사전 요구사항), Section 3(호스트 환경 설정), Section 5.8.4(Jenkins 에이전트), Section 7.W(트러블슈팅)는 **Windows 전용**으로 작성되었다.
...
| RAM | 최소 16 GB (GitLab + Docker + Ollama 동시 실행 기준) |
| 디스크 | 최소 50 GB 여유 공간 (SSD 권장) |
| GPU | NVIDIA GPU 8GB VRAM 이상 + 최신 드라이버 (본 가이드는 **RTX 4070 Mobile 8GB** 기준으로 작성됨. Ollama가 자동 감지하여 GPU 가속 사용) |
...
```powershell
# 필수 모델 다운로드 (8GB VRAM 환경에 최적화된 모델)
...
> **참고:** `qwen3:8b` 모델은 약 5 GB, `llama3.2-vision`은 약 4 GB의 디스크/VRAM 공간이 필요하다. RTX 4070 Mobile(8GB VRAM)에서 단일 모델은 완전히 GPU에 로드되지만, **두 모델을 동시에 올릴 수는 없다.** 사용하지 않는 모델은 자동으로 언로드된다.

4. **Ollama API 포트 확인:**

Ollama는 기본적으로 `http://localhost:11434`에서 서비스한다. Docker 컨테이너에서는 `http://host.docker.internal:11434`로 접근한다.

#### Step 4-1: GPU 최적화 설정 (Windows)

Ollama는 호스트 Windows에서 네이티브로 실행되므로 Docker GPU 패스스루 없이 직접 GPU를 사용한다. 최대 성능을 끌어내려면 아래 설정을 확인한다.

**1. Windows 전원 계획을 "고성능"으로 변경한다.**

LLM 추론 중 CPU/GPU가 절전 모드로 클럭을 낮추면 응답 속도가 급격히 느려진다.
...
# 또는 "최고 성능" 모드 활성화 (Windows 11)
powercfg -duplicatescheme e9a42b02-d5df-448d-aa00-03f14749eb61
powercfg /setactive e9a42b02-d5df-448d-aa00-03f14749eb61
```

**2. NVIDIA GPU 드라이버를 최신 버전으로 유지한다.**
...
> **드라이버 선택:** 게임 최적화가 아닌 안정성이 중요하므로 **NVIDIA Studio Driver**를 권장한다. https://www.nvidia.com/drivers 에서 다운로드.

**3. Ollama GPU 환경변수를 설정한다.**

시스템 환경변수에 아래 값을 추가하면 Ollama의 GPU 활용을 세밀하게 제어할 수 있다.

| 환경변수 | 설명 | 권장값 |
| --- | --- | --- |
| `OLLAMA_GPU_LAYERS` | 모델의 몇 개 레이어를 GPU VRAM에 올릴지 결정. `-1`이면 전체 모델을 GPU에 로드 | `-1` (8B 모델은 8GB VRAM에 완전 적재 가능) |
| `OLLAMA_KEEP_ALIVE` | 모델을 VRAM에 유지하는 시간. **8GB VRAM에서는 짧게 설정**하여 다른 모델이 로드될 여유를 확보 | `5m` |
| `OLLAMA_NUM_PARALLEL` | 동시 요청 처리 수. **8GB VRAM에서는 1 고정** (병렬 시 OOM 발생) | `1` |
| `OLLAMA_FLASH_ATTENTION` | Flash Attention 활성화. 추론 속도 향상 및 VRAM 사용량 감소 | `1` |

```powershell
# 시스템 환경변수로 영구 설정 (관리자 PowerShell)
[System.Environment]::SetEnvironmentVariable("OLLAMA_GPU_LAYERS", "-1", "Machine")
...
# 설정 후 Ollama 재시작 (시스템 트레이 → Quit → 재실행)
```

> **8GB VRAM 운용 주의사항:**
> - `qwen3:8b`(~5GB)와 `llama3.2-vision`(~4GB)을 **동시에 VRAM에 올릴 수 없다.** `OLLAMA_KEEP_ALIVE=5m`으로 설정하면 5분간 미사용 시 자동 언로드되어 다른 모델이 로드될 수 있다.
> - **지식주입(Vision) 파이프라인과 AI평가 파이프라인을 동시에 실행하지 않는다.** 두 파이프라인이 서로 다른 모델을 사용하므로 VRAM 경합이 발생한다.
> - 평가 실행 전 `ollama ps`로 현재 로드된 모델을 확인하고, 불필요한 모델이 있으면 `ollama stop <모델명>`으로 언로드한다.

**4. `.wslconfig`로 WSL 2 리소스를 제한한다.**

Docker Desktop은 WSL 2 백엔드를 사용하므로, WSL 2가 호스트 메모리를 과도하게 점유하면 Ollama의 GPU VRAM 할당에 영향을 줄 수 있다. `C:\Users\<Username>\.wslconfig` 파일을 생성하여 리소스를 제한한다.

```ini
[wsl2]
# WSL 2 VM에 할당할 최대 메모리 (호스트 RAM의 50% 이하 권장)
memory=6GB
# WSL 2 VM에 할당할 CPU 코어 수
processors=4
# 스왑 파일 크기
swap=4GB
```

> **핵심 원칙:** Ollama는 Windows 네이티브에서 GPU를 직접 사용하고, Docker(WSL 2)는 CPU/메모리만 사용한다. 따라서 WSL 2의 메모리를 제한하여 **GPU VRAM과 호스트 RAM을 Ollama에 최대한 확보**하는 것이 성능 최적화의 핵심이다.

**5. GPU 사용 현황을 모니터링한다.**

평가 파이프라인 실행 중 GPU가 실제로 활용되고 있는지 확인한다.

```powershell
# 실시간 GPU 모니터링 (1초 간격 갱신)
nvidia-smi -l 1

# Ollama에 로드된 모델과 GPU 사용량 확인
ollama ps

# 기대 출력: qwen3:8b 모델이 GPU에 로드되어 있어야 한다
# NAME       ID    SIZE   PROCESSOR    UNTIL
# qwen3:8b   ...   5 GB   100% GPU     5 minutes from now
```

> **확인 포인트:** `PROCESSOR` 컬럼이 `100% GPU`이면 전체 모델이 VRAM에 올라간 상태다. `50% GPU / 50% CPU`처럼 분할되면 VRAM이 부족하여 일부가 시스템 RAM으로 넘어간 것이다. 이 경우 `OLLAMA_GPU_LAYERS` 값을 줄이거나 더 작은 모델을 사용한다.
...
- **파라미터 설명**:
  ...
  - `JUDGE_MODEL`: Active Choices 드롭다운에서 `OLLAMA_BASE_URL/api/tags` 목록을 읽어 선택합니다. 실행 전에 동일 목록으로 재검증합니다. 예: `qwen3:8b`, `qwen2.5-coder:7b`
...
- **파이프라인 단계**:
  ...
  2. **Judge Model 검증**: `OLLAMA_BASE_URL/api/tags`를 다시 조회해 설치된 모델 목록을 로그에 출력하고, 선택한 `JUDGE_MODEL`이 실제로 설치되어 있는지 검증합니다. `JUDGE_MODEL`이 비어 있으면 설치된 모델 중 우선순위(`qwen3:8b` → `qwen2.5-coder:7b` → 첫 번째 모델)로 자동 선택하여 `.effective_judge_model` 파일에 기록합니다.
...
properties([
    parameters([
        ...
        [
            ...
            description: 'OLLAMA 모델 목록에서 선택. 예: qwen3:8b',
            filterLength: 1,
            filterable: true,
            name: 'JUDGE_MODEL',
            ...
            script: [
                ...
                script: [
                    ...
                    script: '''
                        ...
                        def defaultModels = ['qwen3:8b', 'qwen2.5-coder:7b']
...
if selected_model:
    ...
    preferred = ["qwen3:8b", "qwen2.5-coder:7b"]
...
이 섹션의 구성: 5.8.1 설계 원칙 → 5.8.2 아키텍처 → 5.8.3 사용자 워크플로우(3가지 진입 경로) → 5.8.4 Jenkins 에이전트 구축 → 5.8.5 Dify Chatflow 설정 → 5.8.6 Python 실행 엔진 전체 코드 → 5.8.7 Jenkins Pipeline → 5.8.8 산출물 → 5.8.9 운영 가이드 → 5.8.10 로드맵
...
| 단계 | Jenkins Stage 명 | 수행 내용 |
|---|---|---|
| 1 | Prepare Output Directory | qa_reports 디렉터리 생성 및 이전 아티팩트 초기화 |
| 2 | Run Zero-Touch QA Agent | autonomous_qa.py 실행, Dify Brain 연동 E2E 테스트 자율 수행 |
| post | 산출물 아카이빙 | scenario.json, 스크린샷, HTML 리포트 등 artifacts 보관 |

#### 스크립트별 기능요약

| 스크립트 | 역할 |
|---|---|
| `mac_local_executor.py` | 자율 QA 엔진 본체: Playwright 브라우저 제어, 7단계 시맨틱 DOM 탐색, 3단계 하이브리드 자가치유, Dify Brain 연동 (Windows 로컬 에이전트에서 실행) |
...
| v3.4 | `index.html` HTML 리포트 자동 생성, `regression_test.py` 자동 생성, Candidate Search 셀렉터 확장(select/hover), `target_url` Dify 전달, detached element 방어, Jenkinsfile v3.4 전면 개편(Dify Brain/Windows Agent), Flow 3 `convert` 모드(Playwright codegen → DSL 변환), `--scenario` 직접 실행 옵션 |
...
│   │                    │  │                  │  │                │  │
│   │  모델: qwen3:8b │  │  모델: GPT-4o    │  │  모델: 가용LLM  │  │
│   └────────┬───────────┘  └────────┬─────────┘  └───────▲────────┘  │
│            │                       │                    │           │
│            └───────────┬───────────┘                    │           │
│                        │ 9대 표준 DSL (JSON)            │           │
...
└─────────────────────────────────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────────┐
│                인프라 계층 (Jenkins + Windows Agent)                  │
│                                                                     │
│   ┌──────────────────┐     ┌─────────────────────────────────────┐  │
│   │  Jenkins Master   │────▶│ Windows Local Agent (windows-ui-   │  │
│   │  (Docker)         │     │  tester)                           │  │
│   │  :8080            │     │  - Java 17 + Playwright Chromium   │  │
│   └──────────────────┘     │  - Headed 브라우저 (GUI 세션)       │  │
│                            │  - Defender SmartScreen/방화벽 허용  │  │
│                            └─────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
> **주의:** 루트 `readme.md`에서는 Label이 `mac-ui-tester`이지만, Windows 환경에서는 `windows-ui-tester`로 설정한다. Jenkinsfile에서 `agent { label 'windows-ui-tester' }`로 설정되어 있는지 확인한다.
...
Zero-Touch QA의 지능 계층은 Dify Chatflow로 구현한다. `run_mode` 변수에 따라 Planner / Vision Refactor / Healer LLM 노드로 분기하여, 3가지 진입 경로(Doc/Chat/Record)와 자가 치유(Heal)를 하나의 앱에서 처리한다.
...
| # | 변수명 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- | --- |
| 1 | `run_mode` | **Select** | 예 | 옵션값: `chat`, `doc`, `record`, `heal`. 실행 흐름 분기용 |
...
| 6 | `is_automated` | **Boolean** | 아니오 | CI/수동 개입 판별 (기본값: true) |

**Select 타입 설정 방법:**

`run_mode` 변수의 타입을 `Select`로 지정한 후, 옵션 목록에 `chat`, `doc`, `record`, `heal` 을 각각 추가한다. 기본값은 `chat`으로 설정한다.
...
- **IF 조건 1:** `run_mode` `is` `chat` **OR** `run_mode` `is` `doc`
  - → Planner LLM 노드로 연결
- **ELIF 조건 2:** `run_mode` `is` `record`
  - → Vision Refactor LLM 노드로 연결
- **ELSE** (그 외, 즉 `heal`):
  - → Healer LLM 노드로 연결

> Dify UI에서 조건 추가는 `+ 조건 추가` 버튼으로 OR 조건을 만들 수 있다. ELIF는 `+ ELIF 추가` 버튼으로 생성한다.
...
3. IF/ELSE 노드의 IF 출력 핸들을 Planner 노드의 입력에 연결한다.

**모델 선택:**

우측 패널 상단에서 모델을 선택한다:
- **권장:** `qwen3:8b` (Ollama 로컬 모델, 코드 생성 특화)
...
##### 5.8.5.5 Vision Refactor LLM 노드 (Flow 3 처리용)

Record 모드에서 브라우저 녹화 데이터를 시맨틱 DSL로 정제하는 노드이다.

1. 캔버스에서 `+ 노드 추가` > **LLM**을 선택한다.
2. 노드 이름을 `Vision Refactor`로 변경한다.
3. IF/ELSE 노드의 ELIF 출력 핸들을 이 노드의 입력에 연결한다.

**모델 선택:**

- **권장:** `GPT-4o` 또는 `Claude Sonnet 4.5` (이미지 인식 필요 — Vision 지원 모델 필수)
- Vision Detail: `High`로 설정한다 (모델 설정에 해당 옵션이 있는 경우).
...
3. 첫 번째 스텝은 반드시 navigate 액션으로 시작하십시오.
...
##### 5.8.5.6 Healer LLM 노드 (에러 복구용)

실행 중 요소 탐색에 실패했을 때, 에러 메시지와 DOM 스냅샷을 분석하여 대체 셀렉터를 찾아주는 노드이다.

1. 캔버스에서 `+ 노드 추가` > **LLM**을 선택한다.
2. 노드 이름을 `Healer`로 변경한다.
3. IF/ELSE 노드의 ELSE 출력 핸들을 이 노드의 입력에 연결한다.

**모델 선택:**

- **권장:** 가용한 최고 성능 모델 (`GPT-4o`, `Claude Sonnet 4.5`, 또는 `qwen3:8b`)
...
##### 5.8.5.7 Answer 노드 — 최종 응답 연결

Chatflow는 반드시 **Answer** 노드가 있어야 API 응답을 반환한다.

1. 캔버스에서 `+ 노드 추가` > **Answer**를 선택한다.
2. 3개의 LLM 노드(Planner, Vision Refactor, Healer)의 출력 핸들을 **모두** 이 Answer 노드의 입력에 연결한다.
3. Answer 노드를 클릭하고, 응답 내용에 LLM 노드의 출력 변수를 참조한다:
   - `{` 키를 누르고 `Planner` 노드의 `text` 출력을 선택한다.
   - 같은 방식으로 `Vision Refactor`, `Healer` 노드의 `text` 출력도 추가한다.
   - IF/ELSE 분기에 의해 실제로는 하나의 LLM만 실행되므로, 세 출력을 모두 넣어도 실행된 노드의 결과만 반환된다.

**최종 캔버스 구조:**

```
Start → IF/ELSE ─── IF (chat/doc) ──→ Planner ──────→ Answer
                 ├── ELIF (record) ──→ Vision Refactor ─┘
                 └── ELSE (heal) ────→ Healer ��─────────┘
```

##### 5.8.5.8 앱 게시 및 API Key 발급
...
| --- | --- |
| `run_mode` | `heal` |
| `error` | `요소 탐색 실패: role=button, name=로그인` |
| `dom` | `<nav><button class="btn-login" aria-label="로그인하기">Sign In</button></nav>` |

**기대 출력 (순수 JSON 객체):**

```json
{"step": 2, "action": "click", "target": "role=button, name=로그인하기", "value": "", "description": "로그인 버튼 클릭 (aria-label 기반 복구)"}
```


#### 5.8.6 통합 실행 엔진 (`mac_local_executor.py`) 전체 소스코드

ZeroTouch QA의 핵심 실행 파일이다. 아래 기능이 하나의 스크립트에 통합되어 있다.

- **DifyBrain 클래스:** Dify Chatflow API와 통신하여 시나리오(DSL)를 받아오고, 에러 발생 시 Heal 요청을 보낸다.
- **LocatorResolver 클래스:** 7단계 시맨틱 탐색으로 Playwright 로케이터를 생성한다.
- **LocalHealer 클래스:** 비용 제로의 로컬 자가 치유(Candidate Search)를 수행한다.
- **QAExecutor 클래스:** 9대 DSL 액션을 실행하고, 3단계 하이브리드 Self-Healing을 관리하며, 산출물을 생성한다.

**파일 경로:** `C:\automation\local_qa\mac_local_executor.py`
...
class DifyBrain:
    ...
    def call_api(self, payload, file_id=None):
        ...
        """
        # Dify Chatflow에서 query는 필수 필드이나, 실제 분기 로직은 inputs의
        # run_mode 값으로 결정된다. query는 형식적 필수값으로 고정 문자열을 사용한다.
        req_body = {
            "inputs": payload,
            "query": "실행을 요청합니다.",
            "response_mode": "blocking",
...
class QAExecutor:
    ...
    def execute(self, scenario, is_ci):
        ...
        with sync_playwright() as p:
            # CI(Jenkins Windows Agent)에서는 headed 모드로 실행하여 화면 증적을 남기고,
            # 로컬 디버깅에서는 headless로 실행한다.
            ...
            try:
                for step in scenario:
                    ...
                    # ── 타겟 필요 액션: 실행 + 3단계 자가 치유 루프 ──
                    # attempt 0: 실행 → (실패) → [Heal 1] Local 유사도 매칭 → (실패) → [Heal 2] Dify LLM → step 수정
                    # attempt 1: 수정된 step 재실행 → (실패) → [Heal 1] Local 재시도 → (실패) → 최종 FAIL
                    max_retries = 2
...
Windows Local 에이전트(5.8.4)에서 `mac_local_executor.py`(5.8.6)를 실행하는 파이프라인이다. Python venv 캐싱, 환경 변수 자동 주입, 산출물 영구 보관이 포함되어 있다.
...
| Job 유형 | Pipeline |
| 실행 노드 | `windows-ui-tester` (Windows Local Agent) |
...
pipeline {
    ...
    environment {
        AGENT_HOME = "C:\\automation\\local_qa"
        ...
        stage('1. 환경 동기화 및 캐싱') {
            steps {
                bat """
                    ...
                    rem 파이썬 Venv 캐시 활용 (최초 실행 시에만 생성)
                    if not exist venv (
        ...
        stage('2. 문서 파일 준비 (Doc 모드)') {
            ...
            steps {
                withFileParameter('DOC_FILE') {
                    bat "copy %DOC_FILE% ${AGENT_HOME}\\upload.pdf"
                }
            }
        }
            ...
            steps {
                script {
                    bat """
                        ...
                        set RUN_MODE=${params.RUN_MODE}
                        set TARGET_URL=${params.TARGET_URL}
                        set SRS_TEXT=${params.SRS_TEXT}

                        set FILE_ARG=
                        if "${params.RUN_MODE}"=="doc" if exist upload.pdf set FILE_ARG=--file upload.pdf

                        echo [Engine] QA 엔진 구동 시작 (RUN_MODE=${params.RUN_MODE})
                        python mac_local_executor.py --mode execute %FILE_ARG%
                    """
                }
            }
        }
...
| # | 항목 | 확인 방법 | 미완료 시 참고 |
| --- | --- | --- | --- |
| 1 | Windows Agent 연결 | Jenkins > Nodes에서 `windows-ui-tester` 상태가 Connected | Section 5.8.4 |
| 2 | Windows 보안 설정 | Defender SmartScreen 예외 등록, 방화벽에서 Java/Python 허용, 절전 모드 해제 | Section 5.8.4.3 |
...
python mac_local_executor.py --mode convert --file recorded.py
```
...
```powershell
python mac_local_executor.py --mode execute --scenario artifacts\scenario.json
```
...
cd C:\automation\local_qa
.\venv\Scripts\Activate.ps1
$env:DIFY_API_KEY = "app-xxxxxxxxx"
$env:RUN_MODE = "chat"
$env:SRS_TEXT = "네이버에서 DSCORE 검색 후 엔터"
$env:TARGET_URL = "https://www.naver.com"

python mac_local_executor.py --mode execute
```
...
$env:DIFY_API_KEY = "app-xxxxxxxxx"
$env:RUN_MODE = "doc"
$env:TARGET_URL = "https://target-app.com"

python mac_local_executor.py --mode execute --file .\requirements_spec.pdf
```

> 로컬 실행 시 `JENKINS_HOME` 환경변수가 없으므로 **headless 모드**로 동작한다. 브라우저 화면을 보려면 `$env:JENKINS_HOME = "1"`을 추가한다.
...
```powershell
cd C:\automation\local_qa\artifacts
...
| 증상 | 원인 | 해결 |
| --- | --- | --- |
| 실행 파일이 차단됨 | Windows Defender SmartScreen 차단 | Windows 보안 > 앱 및 브라우저 컨트롤 > SmartScreen에서 예외 등록 |
| 네트워크 연결 실패 | Windows 방화벽 차단 | Windows Defender 방화벽 > 허용된 앱에서 Java, Python 인바운드/아웃바운드 허용 |
...
> **Windows 참고:** PowerShell에서 여러 줄 명령은 백틱(`` ` ``)을 줄바꿈 문자로 사용한다. Git Bash에서는 macOS와 동일하게 `\`를 사용한다.
...
**해결:**
```powershell
# 호스트에서 Ollama 실행 확인 (Windows)
Get-Process ollama -ErrorAction SilentlyContinue

# Ollama가 실행되지 않으면 시작 메뉴에서 Ollama를 실행하거나:
ollama serve
...
# GPU 인식 상태 확인
ollama ps
# PROCESSOR 컬럼이 "100% CPU"이면 GPU 미사용

# NVIDIA 드라이버 확인
nvidia-smi
# 출력이 없거나 에러면 드라이버 미설치
```

**해결:**
1. https://www.nvidia.com/drivers 에서 최신 Studio Driver를 다운로드하여 설치한다.
2. PC를 재부팅한다.
3. Ollama를 재시작한다 (시스템 트레이 아이콘 → Quit → 재실행).
4. `nvidia-smi` 및 `ollama ps`로 GPU 인식을 재확인한다.

**문제: VRAM 부족으로 모델이 GPU/CPU 분할 로드된다**

**진단:**
```powershell
ollama ps
# PROCESSOR: "50% GPU / 50% CPU" → VRAM 부족
```

**해결:**
1. 불필요한 GPU 프로세스 종료 (게임, 브라우저 하드웨어 가속 등):
```powershell
# GPU 메모리 사용 프로세스 확인
nvidia-smi --query-compute-apps=pid,name,used_memory --format=csv
```
2. `OLLAMA_GPU_LAYERS` 값을 줄여 일부만 GPU에 올린다:
```powershell
[System.Environment]::SetEnvironmentVariable("OLLAMA_GPU_LAYERS", "40", "Machine")
# Ollama 재시작
```
3. 더 작은 모델 사용을 고려한다 (예: `qwen2.5-coder:7b` → 약 4.5 GB VRAM).

**문제: 평가 실행 중 Ollama가 느려지거나 응답이 없다**

**원인:**
- GPU 온도 과열로 스로틀링이 발생했다.
- 모델이 VRAM에서 언로드되었다.

**해결:**
```powershell
# GPU 온도 확인 (90°C 이상이면 스로틀링 발생)
nvidia-smi --query-gpu=temperature.gpu --format=csv

# 모델 유지 시간 연장
[System.Environment]::SetEnvironmentVariable("OLLAMA_KEEP_ALIVE", "60m", "Machine")

# Ollama 재시작 후 모델 미리 로드
ollama run qwen3:8b ""
```
