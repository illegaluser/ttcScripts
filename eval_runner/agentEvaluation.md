...
- **파이프라인 단계**:
  ...
  2. **Judge Model 검증**: `OLLAMA_BASE_URL/api/tags`를 다시 조회하여 현재 설치된 모델 목록을 로그에 출력하고, 사용자가 고른 `JUDGE_MODEL`이 실제 목록에 없으면 즉시 실패시킵니다.
  3. **파이썬 평가 실행**: `pytest`를 사용하여 `test_runner.py`를 실행합니다. 이때 모든 파라미터와, 존재하는 경우 암호화된 `langfuse` 인증키가 환경변수로 주입됩니다.
  4. **결과 보고**: 파이프라인 실행이 끝나면, `post` 단계에서 `/var/knowledges/eval/reports` 산출물을 워크스페이스 `eval-reports/` 로 복사하고 `summary.html`, `summary.json`, `results.xml`을 Jenkins 아티팩트로 보관합니다.

- **Active Choices 사용 시 주의사항**:
  - Jenkins에 `uno-choice` 플러그인이 설치되어 있어야 합니다.
  - Jenkinsfile을 처음 저장한 직후 한 번은 파라미터 정의만 반영되고, 다음 실행부터 `JUDGE_MODEL` 드롭다운이 정상 표시될 수 있습니다.
  - 동적 파라미터 스크립트는 Jenkins Script Security 정책에 따라 관리자 승인 또는 비샌드박스 실행이 필요할 수 있습니다.
...
properties([
    parameters([
        ...
        password(name: 'TARGET_AUTH_HEADER', defaultValue: '', description: '(선택) 전체 인증 헤더. 예: Authorization: Bearer xxx'),
