...
- **파라미터 설명**:
  ...
  - `TARGET_TYPE`: 평가 방식입니다. API면 `http`, 웹 채팅 UI면 `ui_chat`를 선택합니다. (`local_ollama_wrapper`, `openai_wrapper`는 `http`에서만 지원)
  ...
  - `API_KEY`: 대상 API가 `x-api-key` 같은 별도 키 인증을 요구할 때 입력합니다. `openai_wrapper`에서는 `OPENAI_API_KEY`가 비었을 때 fallback으로도 사용됩니다.
...
properties([
    parameters([
        ...
        choice(name: 'TARGET_MODE', choices: ['local_ollama_wrapper', 'openai_wrapper', 'direct'], description: '대상 호출 방식 선택'),
