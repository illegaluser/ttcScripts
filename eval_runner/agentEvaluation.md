...
- **파이프라인 단계**:
  ...
  3. **Target URL 연결 검증**: 평가 전에 Jenkins 컨테이너에서 `TARGET_URL` 접근 가능 여부를 먼저 확인합니다. 연결 불가일 경우 `HTTPConnectionPool ... [Errno 101] Network is unreachable` 유형의 원인을 즉시 표시하고 평가를 중단합니다.
  4. **파이썬 평가 실행**: `pytest`를 사용하여 `test_runner.py`를 실행합니다. 이때 모든 파라미터와, 존재하는 경우 암호화된 `langfuse` 인증키가 환경변수로 주입됩니다.
  5. **결과 보고**: 파이프라인 실행이 끝나면, `post` 단계에서 `/var/knowledges/eval/reports` 산출물을 워크스페이스 `eval-reports/` 로 복사하고 `summary.html`, `summary.json`, `results.xml`을 Jenkins 아티팩트로 보관합니다.
