...
- **파라미터 설명**:
  ...
  - `GEMINI_MIN_INTERVAL_SEC`: Gemini 호출 최소 간격(초)입니다. 429가 잦으면 `1.5~3.0`으로 올립니다.
  - `GEMINI_MAX_RETRIES`: Gemini 429 재시도 횟수입니다. 예: `5~8`
  - `GEMINI_RETRY_BASE_SEC`: Gemini 백오프 시작 지연(초)입니다. 예: `1.0`
  - `GEMINI_RETRY_MAX_SEC`: Gemini 백오프 최대 지연(초)입니다. 예: `20.0~60.0`
...
properties([
    parameters([
        ...
        string(name: 'GEMINI_MIN_INTERVAL_SEC', defaultValue: '1.0', description: 'Gemini 호출 최소 간격(초)'),
        string(name: 'GEMINI_MAX_RETRIES', defaultValue: '5', description: 'Gemini 429 재시도 횟수'),
        string(name: 'GEMINI_RETRY_BASE_SEC', defaultValue: '1.0', description: 'Gemini 백오프 시작 지연(초)'),
        string(name: 'GEMINI_RETRY_MAX_SEC', defaultValue: '20.0', description: 'Gemini 백오프 최대 지연(초)'),
        [
...
GEMINI_RETRY_BASE_SEC   = 1.0
GEMINI_RETRY_MAX_SEC    = 30.0
JUDGE_MODEL             = qwen3-coder:30b (심판 모델)
OLLAMA_BASE_URL         = http://host.docker.internal:11434 (심판 모델용)
GOLDEN_CSV_PATH         = /var/knowledges/eval/data/golden.csv
```
...
> 429 오류가 반복될 때
> - 의미: Gemini API 쿼터 초과 또는 초당/분당 호출 제한 초과입니다.
> - 조치 1: Google AI Studio/Cloud 프로젝트의 API 사용량 한도와 결제(Billing) 상태를 확인합니다.
> - 조치 2: `GEMINI_MODEL`을 더 가벼운 모델로 변경하거나, golden set 크기를 줄여 호출량을 낮춥니다.
> - 조치 3: 필요 시 빌드를 잠시 쉬고 재시도합니다. (`gemini_wrapper`에는 429 자동 재시도/백오프가 적용됨)
