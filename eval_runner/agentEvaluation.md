...
- **파라미터 설명**:
  ...
  - `AUTO_MANAGE_WRAPPER`: `true`이면 `TARGET_URL`이 `:8000/invoke`일 때 Jenkins 컨테이너 내부에서 wrapper를 자동으로 기동하고 빌드 종료 시 자동 종료합니다.
...
- **파이프라인 단계**:
  ...
  3. **Target URL 연결 검증 / Wrapper 준비**: `AUTO_MANAGE_WRAPPER=true` 이고 `TARGET_URL`이 `:8000/invoke`이면 wrapper를 자동 기동한 뒤 연결 검증을 수행합니다. IPv4/IPv6 주소별 TCP 연결 결과를 모두 출력해 `Connection refused`(서비스 미기동)와 `Network is unreachable`(라우팅 문제)를 구분해 보여줍니다.
...
properties([
    parameters([
        ...
        booleanParam(name: 'AUTO_MANAGE_WRAPPER', defaultValue: true, description: 'TARGET_URL이 :8000/invoke일 때 로컬 wrapper 자동 기동/종료'),
